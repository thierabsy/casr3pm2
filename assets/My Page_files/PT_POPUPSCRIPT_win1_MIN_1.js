...
ShowAccessibilityPopup:function() 
{
 ...
 this.focusElem=null; bMOPopupDisplay_win1=true; for (var i=0; i<document.forms.length; i++)
 {
...
{
 this.hoverId=hoverId; this.hoverpopupId=popupId;  if (bAccessibility_win1) 
 {
 ...
 this.MOcurrHeight = this.nDefHeight;  this.tHandle=setTimeout("MOpopupObj_win1.ShowPopup()",this.nOpenDelay * 1000); }
},
...
StopPopup:function(bNoDelay) 
{
 ...
 hItem.style.cursor = "";  }
 this.tHandle = null; if ((bAccessibility_win1) || (typeof bNoDelay !="undefined")) 
 this.HidePopup(); else
 this.tHandle=setTimeout("MOpopupObj_win1.HidePopup()", 1000 * this.nCloseDelay);},

HidePopup:function() 
{
 var PopContainer = document.getElementById(this.MOPopContainer);  PopContainer.style.visibility = "Hidden"; PopContainer.style.zIndex="1"; this.bIs_shown = false; if (bAccessibility_win1) 
 {
...
ShowAccessibilityPopup:function(winname) 
{
 ...
 this.focusElem=null; bHelpPopupDisplay_win1=true; for (var i=0; i<document.forms.length; i++)
 {
...
StartPopup:function(evt,hoverId,popupId, nWidth, nHeight, labeltext, helptext, winname) 
{
 ...
 ptEvent.add(HelppopTitle,"mousedown",function(evt){HelppopupObj_win1.InitMovePopup(evt,winname)});  ptEvent.add(document,"mouseup",function(evt){HelppopupObj_win1.StopMovePopup(evt,winname)}); ptEvent.add(document,"mousemove",function(evt){HelppopupObj_win1.MovePopup(evt,winname)}); } 
 if(!bWSRP) 
 ...
 this.HelpcurrHeight = this.nDefHeight;  HelppopupObj_win1.ShowPopup(evt,winname);},
...
ShowPopup:function(evt,winname) 
{
 ...
 HelppopupObj_win1.topDiff = hp.offsetParent.offsetTop; HelppopupObj_win1.leftDiff = hp.offsetParent.offsetLeft; }
 if (document.activeElement != null && document.activeElement.tagName != null && document.activeElement.tagName.indexOf("BODY") <0) {

 this.focusElem = document.activeElement;  if (this.focusElem != null)
 this.focusElem.blur(); }
 else
 this.focusElem=null; HelppopupObj_win1.SetHeader(winname); HelppopupObj_win1.SetShadow(winname); var HelppopContainer = document.getElementById("HelppopupContainer_"+winname); var HelppopFrame = document.getElementById("HelpdivpopupFrame_"+winname); HelppopFrame.focus(); ptEvent.add(HelppopContainer,"keypress",function(evt){HelppopupObj_win1.HandleEscapeKey(evt,winname)}); cancelBubble(evt);},
...
HandleEscapeKey:function(evt,winname) {
 ...
 ptEvent.remove(hp,"keypress",HelppopupObj_win1.HandleEscapeKey); cancelBubble(evt); HelppopupObj_win1.StopPopup(winname);},
...
StopPopup:function(winname) 
{
 ...
 var hItem = document.getElementById(this.hoverId);  if (hItem)
 hItem.style.cursor = "";  }
 if ((typeof bAccessibility_win1 != 'undefined') && (bAccessibility_win1)) 
 {
...
InitMovePopup:function(e,winname){
 var HelppopContainer = document.getElementById("HelppopupContainer_"+winname);  if(document.all) e = event; if(document.all) e = event;  var HelppopInner = document.getElementById("HelppopupInner_"+winname);  var HelpDragframe = document.getElementById('HelpDragFrame_'+winname);  var XYposition = ptCommonObj2.getPosition(HelppopInner);  HelppopupObj_win1.moveCounter = 1; HelppopupObj_win1.startEventPos = [e.clientX,e.clientY]; HelppopupObj_win1.startPosWindow = [XYposition.x,XYposition.y];  if (HelpDragframe) {
...
StopMovePopup:function(e,winname) {
 ...
 var HelppopContainer = document.getElementById("HelppopupContainer_"+winname);  var XYposition = ptCommonObj2.getPosition(document.getElementById('HelpDragFrame_'+winname)); if(document.all) e = event; var tdiff = HelppopupObj_win1.topDiff; var ldiff = HelppopupObj_win1.leftDiff; var cW = HelppopContainer.clientWidth; var cH = HelppopContainer.clientHeight; var bW = document.body.clientWidth; var bH = document.body.clientHeight; if(XYposition.x + cW > bW)
...
MovePopup:function(e,winname) {
 if(document.all)
 e = event; var HelpDragframe = document.getElementById('HelpDragFrame_'+winname);  if(HelppopupObj_win1.moveCounter>=1 && HelpDragframe){ 
 var mX = e.clientX; var mY = e.clientY; var cX = HelppopupObj_win1.actualStartPos[0]; var cY = HelppopupObj_win1.actualStartPos[1]; var tdiff = HelppopupObj_win1.topDiff; var ldiff = HelppopupObj_win1.leftDiff; var aY = 0; HelpDragframe.style.left = HelppopupObj_win1.startPosWindow[0] + mX - HelppopupObj_win1.startEventPos[0] + 'px';  HelpDragframe.style.top = ((HelppopupObj_win1.startPosWindow[1] + mY - HelppopupObj_win1.startEventPos[1])-aY) + 'px';  }
},
...
UpdatePopup:function(evt,sHTML,winname) 
{
 ...
 this.HelpcurrWidth = viewportWidth; HelppopupObj_win1.actualStartPos = [evt.clientX,evt.clientY];  var nLeft=this.GetElementleft(evt, hItem);  if(nLeft < 3)
 nLeft = 3; if('ltr' == 'ltr') {
 nLeft = nLeft + 20; }
 else {
 if(this.bIE)
...
var MOpopupObj_win1 = new PT_MOpopup(); MOpopupObj_win1.init();var HelppopupObj_win1 = new PT_Helppopup(); HelppopupObj_win1.init();