...
{
var waitobj = document.getElementById("WAIT_win5");var saveobj = document.getElementById("SAVED_win5");var saveWaitObj = document.getElementById("saveWait_win5");updateWindowTitleFromFakeBC(); if (opt == 0)
{
...
if (opt == 2)
{
 showhide(waitobj, false); setSaveText_win5('Saved'); if (saveWaitObj)
 saveWaitObj.style.display = "none"; bTimer = setTimeout("processing_win5(0)",waittime);  if ((typeof top.ptaiAguide=='object') && top.ptaiAguide.ptaiItemId!=null && top.ptaiAguide.ptaiItemId)
 {AgPostMessage();}
}
}
...
var nLastKey_win5 = 0;var oLastKeyEvent = null;var gKeyDownTarget = null;function doKeyDown_win5(evt)
{
oLastKeyEvent = evt;var target = getEventTarget(evt);gKeyDownTarget = target;if (!isFModeLayout() && !PT_handleTabKeyForModalDialog(evt))
 ...
 return findHandler("doKeyDown_" + target.form.name, evt);nLastKey_win5 = getKeyCode(evt);var bKeyUp = target && target.getAttribute('onkeyup') != null;if (isFModeLayout() && nLastKey_win5 == 13 && bKeyUp)
 evt.preventDefault();if (nLastKey_win5 != "\t".charCodeAt(0)) {

if (isGridNav(evt))
 ptGridObj_win5.doArrowNavGrid(evt);if (isTypeAheadEl(evt))
 ptTAObj_win5.KeyUpDown(evt);if(browserInfoObj2.isSafari && nLastKey_win5 == 27){
...
if (keyChar == "\r" && (!(isButton(target) || isRowAct(target)) || !isFModeLayout()) || key == 27)
 ...
 if (key == 27)
 {
 if (typeof (bDoModal_win5) != "undefined" && bDoModal_win5 && modalID != null) 
 doUpdateParent(document.win5, '#ICCancel');  else if (isFModeLayout() && bInModal)
 submitAction_win5(document.win5, '#ICCancel'); return; } 
...
{ 
 ...
 submitAction_win5(document.win5, event.srcElement.id); else 
 submitAction_win5(document.win5, evt.target.id);}else
{
...
window.doKeyPress_win5 = function (evt)
 ...
 addchg_win5(target); if (isFModeLayout() && target && (target.getAttribute('onkeyup') != null
 || target.getAttribute('onkeydown') != null))
 return; if (target && target.form && target.form.name != "win5")
 ...
 submitAction_win5(document.win5, "#KEY" + code); return false; }
}
...
{
 var currentNode = document.getElementById(document.win5.name+"div"+fldname); if(currentNode)
 {
 ...
 var element ; for (var i = 0; i < document.win5.elements.length; i++)
 {
 element = document.win5.elements[i]; if( element && element.tabIndex > 0 && element.type != "hidden" ) 
 { 
 try{
 tryFocus0New(element);  break;  }
 catch(e){
...
function tryFocus0New(obj)
{
 ...
 ptGridObj_win5.doScrollOnFocus(gn, obj); return false; }
return true;}
...
{
var obj;if ((typeof bAccessibility_win5 != 'undefined') && (bAccessibility_win5)) 
 {
 obj = document.getElementById("MessageTitle_win5"); if (obj)
 return tryFocus(obj); }
...
function UpdateBreadCrumbs() {
 ...
 bcUpdater.updateBreadCrumbs(document.win5); bcUpdater.removeRemoteData();  backNavigation.setCookie(); if (!isFModeLayout()) {
 updClassicHistory()
 }
...
function aAction0_win5(form, name, params, bAjax, bPrompt)
{
if (isFModeLayout() && isPopupRequest(form, name)) return;if (typeof bSearchDialog_win5 != 'undefined' && !bDoModal_win5 && bSearchDialog_win5 && form.ICAction.value.indexOf("\x1b") >=0) 
 {
...
if (typeof ptGridObj_win5 != 'undefined' && ptGridObj_win5)
 ptGridObj_win5.saveScrollPos();form.ICResubmit.value=nResubmit;form.ICAction.value=name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();bcUpdater.getAdvSrchLblFrmCriteria(name, form);var waitingObjList = null;if (loader != null && loader.GetWaitingICAction() != "") 
 ...
 loader=new net2.ContentLoader(postUrl_win5,form,name,null,null,null,null,null,bAjax, bPrompt);else
 loader=new net2.ContentLoader(postUrl_win5,form,name,null,null,null,params,null,bAjax, bPrompt);if (waitingObjList != null)
...
{
 aAction_win5(form, name, params); var searchObj = document.getElementById('PT_SEARCH'); addClass(searchObj, 'side'); addClose(searchObj); SetSearchStatus(obj, form);}
...
function doModalOnLoad_win5(bStartUp, bFileDetach, bNoParent) {
 ...
 && typeof winParent.document.win5 != 'undefined')
...
function doModalMFormSubmit_win5(form, name) {
 ...
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win5.ICStateNum.value = document.win5.ICStateNum.value; winParent.aAction_win5(winParent.document.win5, '#ICCancel'); window.winParent.modWin = null; closeModal(window.modalID); return; }
...
processing_win5(1, 3000);var sFormName = "win5";OpenRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML,sFormName);if (nOpenMode == "0")
...
function DisplayNUIRCMenu_win5(FieldName,nLeft,nTop,sMenu) {
 ...
 var mDivObj = document.getElementById('win5' + 'div' + FieldName + '$divpop'); if (!mDivObj)
