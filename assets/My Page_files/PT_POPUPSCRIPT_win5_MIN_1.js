...
ShowAccessibilityPopup:function() 
{
 ...
 this.focusElem=null; bMOPopupDisplay_win5=true; for (var i=0; i<document.forms.length; i++)
 {
...
{
 this.hoverId=hoverId; this.hoverpopupId=popupId;  if (bAccessibility_win5) 
 {
 ...
 this.MOcurrHeight = this.nDefHeight;  this.tHandle=setTimeout("MOpopupObj_win5.ShowPopup()",this.nOpenDelay * 1000); }
},
...
StopPopup:function(bNoDelay) 
{
 ...
 hItem.style.cursor = "";  }
 this.tHandle = null; if ((bAccessibility_win5) || (typeof bNoDelay !="undefined")) 
 this.HidePopup(); else
 this.tHandle=setTimeout("MOpopupObj_win5.HidePopup()", 1000 * this.nCloseDelay);},

HidePopup:function() 
{
 var PopContainer = document.getElementById(this.MOPopContainer);  PopContainer.style.visibility = "Hidden"; PopContainer.style.zIndex="1"; this.bIs_shown = false; if (bAccessibility_win5) 
 {
...
ShowAccessibilityPopup:function(winname) 
{
 ...
 this.focusElem=null; bHelpPopupDisplay_win5=true; for (var i=0; i<document.forms.length; i++)
 {
...
StartPopup:function(evt,hoverId,popupId, nWidth, nHeight, labeltext, helptext, winname) 
{
 ...
 ptEvent.add(HelppopTitle,"mousedown",function(evt){HelppopupObj_win5.InitMovePopup(evt,winname)});  ptEvent.add(document,"mouseup",function(evt){HelppopupObj_win5.StopMovePopup(evt,winname)}); ptEvent.add(document,"mousemove",function(evt){HelppopupObj_win5.MovePopup(evt,winname)}); } 
 if(!bWSRP) 
 ...
 this.HelpcurrHeight = this.nDefHeight;  HelppopupObj_win5.ShowPopup(evt,winname);},
...
ShowPopup:function(evt,winname) 
{
 ...
 HelppopupObj_win5.topDiff = hp.offsetParent.offsetTop; HelppopupObj_win5.leftDiff = hp.offsetParent.offsetLeft; }
 if (document.activeElement != null && document.activeElement.tagName != null && document.activeElement.tagName.indexOf("BODY") <0) {

 this.focusElem = document.activeElement;  if (this.focusElem != null)
 this.focusElem.blur(); }
 else
 this.focusElem=null; HelppopupObj_win5.SetHeader(winname); HelppopupObj_win5.SetShadow(winname); var HelppopContainer = document.getElementById("HelppopupContainer_"+winname); var HelppopFrame = document.getElementById("HelpdivpopupFrame_"+winname); HelppopFrame.focus(); ptEvent.add(HelppopContainer,"keypress",function(evt){HelppopupObj_win5.HandleEscapeKey(evt,winname)}); cancelBubble(evt);},
...
HandleEscapeKey:function(evt,winname) {
 ...
 ptEvent.remove(hp,"keypress",HelppopupObj_win5.HandleEscapeKey); cancelBubble(evt); HelppopupObj_win5.StopPopup(winname);},
...
StopPopup:function(winname) 
{
 ...
 var hItem = document.getElementById(this.hoverId);  if (hItem)
 hItem.style.cursor = "";  }
 if ((typeof bAccessibility_win5 != 'undefined') && (bAccessibility_win5)) 
 {
...
InitMovePopup:function(e,winname){
 var HelppopContainer = document.getElementById("HelppopupContainer_"+winname);  if(document.all) e = event; if(document.all) e = event;  var HelppopInner = document.getElementById("HelppopupInner_"+winname);  var HelpDragframe = document.getElementById('HelpDragFrame_'+winname);  var XYposition = ptCommonObj2.getPosition(HelppopInner);  HelppopupObj_win5.moveCounter = 1; HelppopupObj_win5.startEventPos = [e.clientX,e.clientY]; HelppopupObj_win5.startPosWindow = [XYposition.x,XYposition.y];  if (HelpDragframe) {
...
StopMovePopup:function(e,winname) {
 ...
 HelppopupObj_win5.moveCounter=0; }
},

MovePopup:function(e,winname) {
 if(document.all)
 e = event; var HelpDragframe = document.getElementById('HelpDragFrame_'+winname);  if(HelppopupObj_win5.moveCounter>=1 && HelpDragframe){ 
 var mX = e.clientX; var mY = e.clientY; var cX = HelppopupObj_win5.actualStartPos[0]; var cY = HelppopupObj_win5.actualStartPos[1]; var tdiff = HelppopupObj_win5.topDiff; var ldiff = HelppopupObj_win5.leftDiff; var aY = 0; HelpDragframe.style.left = HelppopupObj_win5.startPosWindow[0] + mX - HelppopupObj_win5.startEventPos[0] + 'px';  HelpDragframe.style.top = ((HelppopupObj_win5.startPosWindow[1] + mY - HelppopupObj_win5.startEventPos[1])-aY) + 'px';  }
},
...
UpdatePopup:function(evt,sHTML,winname) 
{
 ...
 this.HelpcurrWidth = viewportWidth; HelppopupObj_win5.actualStartPos = [evt.clientX,evt.clientY];  var nLeft=this.GetElementleft(evt, hItem);  if(nLeft < 3)
 nLeft = 3; if('ltr' == 'ltr') {
 nLeft = nLeft + 20; }
 else {
 if(this.bIE)
...
var MOpopupObj_win5 = new PT_MOpopup(); MOpopupObj_win5.init();var HelppopupObj_win5 = new PT_Helppopup(); HelppopupObj_win5.init();