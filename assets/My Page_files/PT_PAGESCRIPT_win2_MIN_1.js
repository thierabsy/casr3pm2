...
function setSaveText_win2(txt)
{
var saveobj = document.getElementById("SAVED_win2");if (saveobj)
 {
 if (txt == "") txt = 'Saving...';  document.getElementById("ptStatusText_win2").innerHTML = txt; showhide(saveobj, true); document.getElementById("saveWait_win2").style.display = ""; }
}

function processing_win2(opt,waittime)
{
var waitobj = document.getElementById("WAIT_win2");var saveobj = document.getElementById("SAVED_win2");var saveWaitObj = document.getElementById("saveWait_win2");updateWindowTitleFromFakeBC(); if (opt == 0)
{
...
if (opt == 2)
{
 showhide(waitobj, false); setSaveText_win2('Saved'); if (saveWaitObj)
 saveWaitObj.style.display = "none"; bTimer = setTimeout("processing_win2(0)",waittime);  if ((typeof top.ptaiAguide=='object') && top.ptaiAguide.ptaiItemId!=null && top.ptaiAguide.ptaiItemId)
 {AgPostMessage();}
}
}
...
var nLastKey_win2 = 0;var oLastKeyEvent = null;var gKeyDownTarget = null;function doKeyDown_win2(evt)
{
oLastKeyEvent = evt;var target = getEventTarget(evt);gKeyDownTarget = target;if (!isFModeLayout() && !PT_handleTabKeyForModalDialog(evt))
 return false;if (target && target.form && target.form.name != "win2")
 return findHandler("doKeyDown_" + target.form.name, evt);nLastKey_win2 = getKeyCode(evt);var bKeyUp = target && target.getAttribute('onkeyup') != null;if (isFModeLayout() && nLastKey_win2 == 13 && bKeyUp)
 evt.preventDefault();if (nLastKey_win2 != "\t".charCodeAt(0)) {

if (isGridNav(evt))
 ptGridObj_win2.doArrowNavGrid(evt);if (isTypeAheadEl(evt))
 ptTAObj_win2.KeyUpDown(evt);if(browserInfoObj2.isSafari && nLastKey_win2 == 27){
 var evt2=document.createEvent('Event'); evt2.initEvent('keypress',true,true); evt2.keyCode=evt.keyCode;  target.dispatchEvent(evt2); }

 return true;}

if (isCtrlKey(evt))
 return tryFocus(oFirstTab_win2);if (isAltKey(evt) || (isShiftKey(evt) && !isGridNav(evt)))
 return true;if (target == oLastTab_win2)
{
if (oFirstTB_win2)
 {
 if (!bTabOverTB_win2)
...
if (oFirstPg_win2)
 {
 if (!bTabOverPg_win2)
 return tryFocus(oFirstPg_win2); }
if (bTabOverNonPS_win2 || oFirstTB_win2 != null || oFirstPg_win2 != null)
 return tryFocus(oFirstTab_win2);return true;}
if (target == oLastTB_win2)
{
 if (oFirstPg_win2)
 {
 if (!bTabOverPg_win2)
 return tryFocus(oFirstPg_win2); }
 if (bTabOverNonPS_win2 || oFirstPg_win2 != null)
 return tryFocus(oFirstTab_win2); return true;}

if (target == oLastPg_win2)
{
 if (bTabOverNonPS_win2)
 return tryFocus(oFirstTab_win2); return true;}


if (nLastKey_win2 == "\t".charCodeAt(0) && isTypeAheadEl(evt))
{
 ptTAObj_win2.KeyUpDown(evt); return true;}

}

function getLastKey_win2()
{
var nTemp = nLastKey_win2;nLastKey_win2 = 0;return nTemp;}

function doKeyUp_win2(evt)
{
...
if (keyChar == "\r" && (!(isButton(target) || isRowAct(target)) || !isFModeLayout()) || key == 27)
 ...
 if (key == 27)
 {
 if (typeof (bDoModal_win2) != "undefined" && bDoModal_win2 && modalID != null) 
 doUpdateParent(document.win2, '#ICCancel');  else if (isFModeLayout() && bInModal)
 submitAction_win2(document.win2, '#ICCancel'); return; } 
...
if (!routeKeyEvent(evt))
 ...
 {
 if (bCtrl || altKey_win2.indexOf(keyChar) < 0)
 {
 ...
 keyChar = String.fromCharCode(key | 0x40); if (ctrlKey_win2.indexOf(keyChar) < 0)
 bNotMine = true; }
 ...
 {
 if (baseKey_win2.indexOf(keyChar) < 0)
 bNotMine = true; }
...
if (bNotMine) {
 ...
 ptTAObj_win2.GiveOptions(evt); return true;}


var code = getModifiers(evt) + keyChar;if (target && target.name)
 document.win2.ICFocus.value = target.name;if (code == "A8")
 if (!doDeleteKey_win2(target))
 return false;if (code == "A\xbf")
{
 if (window.FindString_win2
 && document.win2.ICFind
 && findScroll(target))
 {
 if (!FindString_win2(document.win2.ICFind))
 return false; }
else
 return false
}
...
{ 
 ...
 doUpdateParent(document.win2, '#ICReturn'); return; }
 ...
 submitAction_win2(document.win2, event.srcElement.id); else 
 submitAction_win2(document.win2, evt.target.id);}else
{
if (target && target.form)
 addchg_win2(target);if (code == 'A5'){
 submitAction_win2(document.win2, "#KEY" + code + operation, target.id);}else if(bAlt){
 if(key == 49) 
 setSaveText_win2('Saving...', true, false);  if(altKey_win2.indexOf(keyChar) >= 0 || keyChar == "2" || keyChar == "3" || keyChar == "4"){
 ...
 var url = "javascript:submitAction_win2(document.win2, \'#KEY" + code + "\')"; if (!saveWarning("", null, "", url))
 return false; }
 if( !ptCommonObj2.isSearchPage(target.form) && (code == "A\xbc" || code == "A\xbe" || code == "A\xde") && gActiveBtn && target.name == null)
 {
 document.win2.ICFocus.value = gActiveBtn.name
 submitAction_win2(document.win2, gActiveBtn.name); }
 else
 submitAction_win2(document.win2, "#KEY" + code);  }
}else{
 submitAction_win2(document.win2, "#KEY" + code);}
}
...
{
var obj = document.getElementById('win2divPAGEBAR');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win2divPSPANELTABS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win2divPSPANELTABLINKS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";}
...
{
return keyHandler_win2(keyCode, bIsAlt, bIsCtrl);}

function keyHandler_win2(keyCode, bIsAlt, bIsCtrl)
{
var keyChar = String.fromCharCode(keyCode);var code = "";if (bIsAlt)
{
 code = "A"; if (bIsCtrl || altKey_win2.indexOf(keyChar) < 0)
 return false;}
else if (bIsCtrl)
{
 code = "C"; keyChar = String.fromCharCode(keyCode | 0x40); if (ctrlKey_win2.indexOf(keyChar) < 0)
 return false;}
else
 return false;code += keyChar;if (code == "A8" || code == "A\xbf" || code == "A7")
 return false;if (target && target.form)
 addchg_win2(target);submitAction_win2(document.win2, "#KEY" + code);return true;}

function doDeleteKey_win2(obj)
{
if (!window.DeleteCheck2_win2)
 return false;var scroll = findScroll(obj);if (!scroll)
 return false;var bFocusable = true;if((obj.nodeName == 'BODY' || obj.id.length == 0) && preID_win2!= null ) {
 var temp = obj.ownerDocument.getElementById(preID_win2); if(typeof temp != 'undefined')
...
if (scroll.id.search(/^(.*)(\$scroll[im]?\$)(.*)$/,"$fdelete$") < 0)
 ...
 return DeleteCheck2_win2(buttonid);if(bFocusable) {
...
var row = bFocusable ? RegExp.$1 : RegExp.$3 - 1 ;if (buttonid.search(/^(.*)(\$fdelete\$)(.*)$/) < 0)
 ...
 return DeleteCheck2_win2(buttonid);return false;}

function doInsertKey_win2(obj)
{
if (!window.AddMultiple_win2)
 return false;var scroll = findScroll(obj);if (!scroll)
...
if (bHasInsertBtn)
{
 ...
 return AddMultiple_win2(document.win2.ICAddCount); if (scroll.id.match(/\$scrolli\$/))
 return true;}
return false;}

function findScroll(obj)
{
var temp = null;if(obj.nodeName == 'BODY' && preID_win2 != null ) {
 temp = obj.ownerDocument.getElementById(preID_win2); if(typeof temp != 'undefined')
 obj=temp;}
...
if (window.doKeyPress_win2 == null)
{
window.doKeyPress_win2 = function (evt)
 {
 oLastKeyEvent = evt; var event = evt ? evt : window.evt; var target = getEventTarget(event); if (target && target.form)
 addchg_win2(target); if (isFModeLayout() && target && (target.getAttribute('onkeyup') != null
 || target.getAttribute('onkeydown') != null))
 return; if (target && target.form && target.form.name != "win2")
 ...
 if (!isFModeLayout() && MOpopupObj_win2 && typeof(MOpopupObj_win2.bIs_shown) != "undefined" && MOpopupObj_win2.bIs_shown)
 return MOpopupObj_win2.StopAccess(event);  if (typeof (bDoModal_win2) != "undefined" && bDoModal_win2)
 ...
 document.win2.ICFocus.value = target.name;   if (key == 13) {
 if (isTypeAheadEl(event) && ptTAObj_win2.IsGrabHighlighted()) {
 return false; } 
 ...
 submitAction_win2(document.win2, "#KEY" + code); return false; }
}
...
var oFirstTab_win2 = null;var oLastTab_win2 = null;var oFirstTB_win2 = null;var oLastTB_win2 = null;var oFirstPg_win2 = null;var oLastPg_win2 = null;var nFirstTBIndex = 5000; var nFirstPgIndex = 10000;function checkTabIndex(obj)
{
if (obj.tabIndex && obj.tabIndex >= 0)
{
 if (obj.tabIndex < nFirstTBIndex)
 {
 if (oLastTab_win2 == null || obj.tabIndex > oLastTab_win2.tabIndex)
 oLastTab_win2 = obj; if (oFirstTab_win2 == null || obj.tabIndex < oFirstTab_win2.tabIndex)
 oFirstTab_win2 = obj; }
 else if (obj.tabIndex < nFirstPgIndex)
 {
 if (oLastTB_win2 == null || obj.tabIndex > oLastTB_win2.tabIndex)
 oLastTB_win2 = obj; if (oFirstTB_win2 == null || obj.tabIndex < oFirstTB_win2.tabIndex)
 oFirstTB_win2 = obj; }
 else
 {
 if (oLastPg_win2 == null || obj.tabIndex > oLastPg_win2.tabIndex)
 oLastPg_win2 = obj; if (oFirstPg_win2 == null || obj.tabIndex < oFirstPg_win2.tabIndex)
 oFirstPg_win2 = obj; }
}
}

function setEventHandlers_win2(sFirst, sLast, bMac)
{
var focus1, focus2;focus1 = function (evt) {doFocus_win2(this, true, true);};focus2 = function (evt) {doFocus_win2(this, false, true);};fchange = function(evt) {addchg_win2(this); };var i;if (sFirst!="")
{
...
var frm = document.win2;if (!frm) return;var frmlen = frm.length;objBeforeOrAfterFocusSave = null; var objBefore = null; var bAfter = 0; var b = navigator.userAgent.toLowerCase();var bIE = browserInfoObj2.isIE;var bIE8 = (bIE && b.indexOf("trident/4.0;") != -1) ? 1:0; for (i = 0; i < frmlen; ++i)
{
...
if (window.setKeyEventHandler_win2 == null)
{
 window.setKeyEventHandler_win2 = function ()
 {
 if (typeof(baseKey_win2) != "undefined")
 {
 document.onkeyup = doKeyUp_win2; if (baseKey_win2.indexOf("\r") >= 0 || baseKey_win2.indexOf("\x1b") >= 0)
 document.onkeypress = doKeyPress_win2; }
 document.onkeydown = doKeyDown_win2; }
}
...
function setFocus_win2(fldname, indx, oDoc0)
{
...
if (bRichTextEnabled == 0)
 ...
 oDoc = document; else
 oDoc = oDoc0;  var obj = null; if (oDoc.win2)
 {
 obj = oDoc.win2.elements[fldname]; if (!obj)
 obj = oDoc.win2[fldname]; }
...
{
 var currentNode = document.getElementById(document.win2.name+"div"+fldname); if(currentNode)
 {
 ...
 var element ; for (var i = 0; i < document.win2.elements.length; i++)
 {
 element = document.win2.elements[i]; if( element && element.tabIndex > 0 && element.type != "hidden" ) 
 { 
 try{
 tryFocus0New(element);  break;  }
 catch(e){
...
function tryFocus0New(obj)
{
 ...
 ptGridObj_win2.doScrollOnFocus(gn, obj); return false; }
return true;}
...
function setFocusSecPageMessage_win2()
{
var obj;if ((typeof bAccessibility_win2 != 'undefined') && (bAccessibility_win2)) 
 {
 obj = document.getElementById("MessageTitle_win2"); if (obj)
 return tryFocus(obj); }
...
function positionWAIT_win2(){
var waitobj = null;var savedobj = null;var objFrame = top.frames['TargetContent'];if (objFrame) {
 waitobj = objFrame.document.getElementById("WAIT_win2"); savedobj = objFrame.document.getElementById("SAVED_win2");}
else {
 waitobj = document.getElementById("WAIT_win2"); savedobj = document.getElementById("SAVED_win2");}
...
{
if (typeof ptGridObj_win2 == "undefined" || !ptGridObj_win2)
 return null;if (typeof gridList_win2 == "undefined" || typeof gridFieldList_win2 == "undefined")
...
for (var i = 0; i < gridList_win2.length; i++)
{
var gn = gridList_win2[i][0];var gc = document.getElementById('divgc'+gn);if (!gc || typeof gc == "undefined") continue;var gfields = gridFieldList_win2[i];for (var j = 0; j < gfields.length; j++)
{
...
{
var gn = this.isGridEl(evt);if (typeof ptGridObj_win2 != "undefined" && ptGridObj_win2 && gn != null)
{
...
function initTypeAheadEl(el) {
 ...
 icAction += "$" + nsuffix; }
 var serverScript = "ptTAObj_win2.oWin.pAction_win2(ptTAObj_win2.oDoc.win2,'" + icAction + "');"; el.obj = oWin.ptTAObj_win2.SetProperties(el, 0, form.ICTypeAheadID, serverScript, true, false, false, true, true, 500);  }
 ...
 return true; else if (typeof PFieldList_win2 != 'undefined' && PFieldList_win2) {
 var fname = removeOccursNum(el.id); for (var i = 0; i < PFieldList_win2.length; i++) {
 if (fname == PFieldList_win2[i][0] || el.id == PFieldList_win2[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 if (ptCommonObj2.isSearchSearchPage(el.form)) {
 if (typeof EFieldList_win2 != 'undefined' && EFieldList_win2) {
 for (var i = 0; i < EFieldList_win2.length; i++) {
 if (fname == EFieldList_win2[i][0] || el.id == EFieldList_win2[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 }
...
function UpdateBreadCrumbs() {
 ...
 bcUpdater.updateBreadCrumbs(document.win2); bcUpdater.removeRemoteData();  backNavigation.setCookie(); if (!isFModeLayout()) {
 updClassicHistory()
 }
...
function GetDomData(form, szTC, szActionName) {
 ...
 return;   var divHiddenFields = document.querySelector("#" + "win2" + "divPSHIDDENFIELDS")
 ...
 form.target = szTC; form.ICAction.value = szActionName; submitAction_win2(form, szActionName);}
...
function preSubmitWorkCenter_win2(arg1, arg2) {
 var isWorkCenter = top.document.getElementById('ptalPgltAreaFrame'); if(!isWorkCenter || !arg1) {return true; }

 if(parent.ptIframe) 
 return preSubmitCheckWorkCenter_win2(arg1,arg2,true);}

function preSubmitCheckWorkCenter_win2(arg1,arg2,bPtreplace) {
if(bPtreplace)
{
...
if (parent.ptIframe.isWorkCenterDirty()) { 
 ...
 tgtFrameForm.ICChanged.value='-1';  submitAction_win2(arg1, arg2); return;  }
 if(parent.ptIframe.saveWarningForWorkCenter(cancelFn,null ,1,arg1, arg2)) 
 return false; }
return true;}
function preSubmitProcessSpellcheck_win2(form, name) {
...
function preSubmitProcessCKEDITOR_win2(form, name) {
 if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField();}

function preSubmitProcess_win2(form, name) {
 if ((name.indexOf('$delete') > 0 || name.indexOf('$srt') > 0) && typeof (preID_win2) != 'undefined' && preID_win2 != null) 
 preID_win2 = null;  if (form == null) {
 form = document.forms['win2']; form.style.display = 'block'; }
 preSubmitProcessSpellcheck_win2(form, name); preSubmitProcessCKEDITOR_win2(form, name);     if (!isFModeLayout()) {
...
function aAction0_win2(form, name, params, bAjax, bPrompt)
{
if (isFModeLayout() && isPopupRequest(form, name)) return;if (typeof bSearchDialog_win2 != 'undefined' && !bDoModal_win2 && bSearchDialog_win2 && form.ICAction.value.indexOf("\x1b") >=0) 
 {
 processing_win2(0, 3000); return; }
...
if (typeof ptGridObj_win2 != 'undefined' && ptGridObj_win2)
 ptGridObj_win2.saveScrollPos();form.ICResubmit.value=nResubmit;form.ICAction.value=name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();bcUpdater.getAdvSrchLblFrmCriteria(name, form);var waitingObjList = null;if (loader != null && loader.GetWaitingICAction() != "") 
 waitingObjList = loader.GetWaitingObjectList();if (typeof bAjax != "undefined" && !bAjax && (typeof params == "undefined" || params == null || params == "") && (typeof popupObj_win2.isModalWidget == "undefined" || !popupObj_win2.isModalWidget))
 form.submit();else if (typeof params == "undefined" || !params)
 loader=new net2.ContentLoader(postUrl_win2,form,name,null,null,null,null,null,bAjax, bPrompt);else
 loader=new net2.ContentLoader(postUrl_win2,form,name,null,null,null,params,null,bAjax, bPrompt);if (waitingObjList != null)
 loader.WaitingObjQueue = waitingObjList;}

function pAction_win2(form, name, efieldname)
{
if(typeof CKEDITOR != 'undefined')UpdateEditorLinkedField();if ((loader != null) && (loader.GetInProcess())) return;form.ICAction.value = name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();if (typeof form.ICTypeAheadID != 'undefined' && form.ICTypeAheadID.value != '') {
 ...
 if (pm)
 pm.updateTypeaheadMsgEvent(name); var flag = form.ICActionPrompt.value; form.ICActionPrompt.value = 'false'; var ret = aAction_win2(form, name, "", true, 1); form.ICActionPrompt.value = flag; return ret;}
else {
 
 if (pm)
 pm.updatePromptMsgEvent(name); form.ICActionPrompt.value = 'true'; return aAction_win2(form, name, "", true, true);}
}

function mAction_win2(form, name, height, width, title, bModalElement, bSizeOnLoad)
{
 form.ICAction.value = name; form.ICXPos.value = ptCommonObj2.getScrollX(); form.ICYPos.value = ptCommonObj2.getScrollY(); aAction_win2(form, name, null, true, false);}





function changeKeyList_win2(obj, form, name, params)
{
 aAction_win2(form, name, params); var searchObj = document.getElementById('PT_SEARCH'); addClass(searchObj, 'side'); addClose(searchObj); SetSearchStatus(obj, form);}



function updateFromPrompt_win2(obj, form, name, params) {
 aAction_win2(form, name, params);}

function updateFromInPrompt_win2(obj, form, name, params) {
 var res = []; var arr1 = document.querySelectorAll(".ps_prompt-resultsgrid .ps_grid-body .psc_on"); for (var i = 0; i < arr1.length; i++)
 res.push(arr1[i].parentElement.parentElement.parentElement.querySelector(".ps_box-value").firstChild.nodeValue); var objHidden = document.getElementById("PT_WORK_PT_PROMPT_TEMP_IN"); objHidden.value = res.join(","); addchg_win2(objHidden); aAction_win2(form, name, params);}
...
function doModlessOnLoad_win2() {
if (window.name.length == 0)
 window.name = "win2";document.win2.target = window.name;}
...
function doModal(url, bPrompt, oParentWin, sCancelName, form, name, sPopupOptions, sResponseHTML) {
 ...
 window.modWin.onLoadExt_win2(); return; }
...
function setModWinParentPIA() {
 if (setModWinParent())
 document.win2.ICResubmit.value = 0;}

function doCancelMsg() {
 aAction_win2(document.win2, '#ICCancel');}
...
function doCancel(sCancelName) {
 ...
 document.win2.ICAction.value = "None"; else {
 ...
 var fm = win.document.win2; if (!fm) fm = window.document.win2; fm.ICAction.value = sCloseId; document.win2.ICAction.value = "None"; }
 }
 try {
 document.win2.target = winParent.name; if (document.win2.ICAction.value != "None") return; if (typeof window.bMFormSubmit != 'undefined')
 winParent.modWinClosed = null; winParent.nResubmit = 0;  if (window.document.win2.enctype.indexOf('multipart') != -1 && typeof window.bMFormSubmit != 'undefined' && !window.bMFormSubmit) {
 winParent.document.win2.ICAction.value = "None"; winParent.MTop().ptCommonObj2.hidePopupMask(); closeModal(modalID); }
 
 else if (sCloseId != "#ICCancel")
 aAction_win2(document.win2, sCloseId); else
 {
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + modalID); if ((typeof modObj != 'undefined') && modObj && modObj.bRCFModal && 
 (typeof(winParent.aAction_win2) == 'undefined'))
 {
 try
 {
 closeModal(modalID); }
 ...
 winParent.aAction_win2(document.win2, sCloseId); }
...
function onParentUnload_win2() {
 if ((typeof window.modWin != 'undefined') && window.modWin) closeModWin_win2();}

function doModalOnLoad_win2(bStartUp, bFileDetach, bNoParent) {
 ...
 && typeof winParent.document.win2 != 'undefined')
 winParent.processing_win2(0, 3000); resizeModalDialog_pt(modalID); } 
 } 
 ...
 if (window.document.win2.enctype.indexOf('multipart') != -1)
 window.bMFormSubmit = false;}

function doUpdateParent(form, name) {
 if (typeof winParent == 'undefined' || winParent == null ) {
 submitAction_win2(document.win2, name); return; }
 if (winParent.bDefer && name.indexOf("#ICCancel") != -1 && bPromptPage_win2) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win2.ICStateNum.value = document.win2.ICStateNum.value; winParent.aAction_win2(winParent.document.win2, '#ICCancel'); var cObj = winParent.document.getElementById("pt_modals"); if(cObj) cObj.style.display = "none"; } else {
 processing_win2(1, 3000); form.target = winParent.name; form.ICAction.value = name; if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField(); winParent.loader = null; if (pm)
  pm.updateParentMsgData(winParent,name);  winParent.aAction_win2(form, name); }
}

function doModalMFormSubmit_win2(form, name) {
 if (name.indexOf("#ICCancel") != -1) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win2.ICStateNum.value = document.win2.ICStateNum.value; winParent.aAction_win2(winParent.document.win2, '#ICCancel'); window.winParent.modWin = null; closeModal(window.modalID); return; }
 window.bMFormSubmit = true;  submitAction_win2(form, name); doModalMFormClose_win2();}

function doModalMFormClose_win2() {
 winParent.bUpLoadProgress = true; winParent.nResubmit = 1; winParent.document.win2.ICStateNum.value = document.win2.ICStateNum.value; winParent.loader = null; winParent.aAction_win2(winParent.document.win2, 'Resubmit'); if (typeof window.winParent != 'undefined' && window.winParent.winParent != null)
...
strCntr+="A";submitAction_win2(document.win2,strCntr);}





var bIsPagelet_win2 = "false", sPageletName_win2 = "";ResetGlyph_win2 = function() {
 if (typeof(sGlyphList_win2) !== "undefined" && sGlyphList_win2 !== "") {
 HideGlyph(sGlyphList_win2); }
}

setPageletInfoInCtxmenu_win2 = function(isPagelet,sPgltName) {
 bIsPagelet_win2 = (isPagelet === "false") ? "false" : "true"; sPageletName_win2 = (typeof(sPgltName) !== "undefined" && sPgltName && sPgltName !== "") ? sPgltName : "";}
...
};var ptTAObj_win2 = new PT_typeahead();function CreateRCMenu4PVG(sPvtGridName,nContext,nLeft,nTop,sActions,sPvgData,BulkActions)
{
...
strAction += "~"+BulkActions;var ch9=String.fromCharCode(9);var elemPVGAct = document.getElementById("ICPVGActMenu"); if (elemPVGAct)
 ...
 elemPVGData.value = sPvgData; submitAction_win2(document.win2,strAction);}

InvokePageAppCls = function(sServname,sData)
{
var sFormName = "win2";var form = document.forms[sFormName];if (typeof form.ICAPPCLSDATA != 'undefined') 
 form.ICAPPCLSDATA.value = sData;var sAppServname = "$APPCLS#"+sServname;submitAction_win2(document.win2,sAppServname);}
...
try {
 ...
 form.ICPVGData.value = sPvgData; var resPTxt = ""; var xmlDoc = null; var sLoader = new net2.ContentLoader(postUrl_win2, form, null, "post",function(){},
...
function doFolder(form, pid, fid) {
 ...
 objHidden.value = fid; addchg_win2(objHidden); submitAction_win2(form, pid);}
...
var bEventContinue= true;var tabSelect;var newTab = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI') 
 ...
 {
 document.win2.ICFocus.value = (tabSelect.firstElementChild || tabSelect.firstChild).id; currentTarget.onclick(); }
 break;  case 39: 
 case 40: 
...
var bReturn = false;var tabSelect = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI')
 ...
 document.win2.ICFocus.value = currentTarget.id.replace("_R", "_L"); else if (currentTarget.id.indexOf("ICTAB_SCROLL_L") == 0)
 document.win2.ICFocus.value = currentTarget.id.replace("_L", "_R"); else if (currentTarget.id.indexOf("ICTAB_HIDE") == 0)
 document.win2.ICFocus.value = currentTarget.id; currentTarget.onclick(); break; case 36: 
...
function InvokeFieldChange_win2(sFldName)
{
 submitAction_win2(document.win2, sFldName);}
function onRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML)
{

processing_win2(1, 3000);var sFormName = "win2";OpenRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML,sFormName);if (nOpenMode == "0")
 processing_win2(0, 3000);}
...
{
 var changed = null; var objFrame = null;  if (document.win2)
 changed = checkFormChanged(document.win2, null);  if ((changed == null || !changed) && top.frames) {
...
{
 if (saveButton == null)
 return; var bZoom = isZoom(); setSaveText_win2('Saving...', true, false); bAutoSave = true; if (bZoom) {
 ...
 var icReturn = oWin.document.getElementById(TOOLS_RETURN); if ((icReturn != null) && (typeof oWin.document.win2.ICAutoSave != "undefined" )) {
 oWin.document.win2.ICAutoSave.value = '1'; icReturn.click(); } 
 }
 }
 
 if (!bZoom) {
 submitAction_win2(document.win2, saveButton); } else {
 
 setTimeout(function(){submitAction_win2(document.win2,saveButton)}, 5000);  }
 }
...
function DisplayNUIRCMenu_win2(FieldName,nLeft,nTop,sMenu) {
 ...
 var mDivObj = document.getElementById('win2' + 'div' + FieldName + '$divpop'); if (!mDivObj)
 ...
 processing_win2(0, 3000); resetMenu(mDivObj); var sClass = mDivObj.getAttribute("class"); sClass = sClass.replace(" psc_hidden", ""); mDivObj.setAttribute("class", sClass); if (typeof(nLeft) != 'undefined' && typeof(nTop) != 'undefined')
 {
...
function DisplayNUIRCMenu(FieldName,sMenu,nLeft,nTop) {
 DisplayNUIRCMenu_win2(FieldName,nLeft,nTop,sMenu);}

InvokeAppClsService_win2 = function(sFldName,sService,sData)
{
var nPos = sFldName.lastIndexOf("$");var szBase = sFldName;if (nPos != -1)
 ...
 szBase = sFldName+"$APPCLS#"+sService;var sFormName = "win2";var form = document.forms[sFormName];if (typeof form.ICAPPCLSDATA != 'undefined') 
 form.ICAPPCLSDATA.value = sData;submitAction_win2(document.win2,szBase);}
