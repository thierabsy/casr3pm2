...
function setSaveText_win4(txt)
{
var saveobj = document.getElementById("SAVED_win4");if (saveobj)
 {
 if (txt == "") txt = 'Saving...';  document.getElementById("ptStatusText_win4").innerHTML = txt; showhide(saveobj, true); document.getElementById("saveWait_win4").style.display = ""; }
}

function processing_win4(opt,waittime)
{
var waitobj = document.getElementById("WAIT_win4");var saveobj = document.getElementById("SAVED_win4");var saveWaitObj = document.getElementById("saveWait_win4");updateWindowTitleFromFakeBC(); if (opt == 0)
{
...
if (opt == 2)
{
 showhide(waitobj, false); setSaveText_win4('Saved'); if (saveWaitObj)
 saveWaitObj.style.display = "none"; bTimer = setTimeout("processing_win4(0)",waittime);  if ((typeof top.ptaiAguide=='object') && top.ptaiAguide.ptaiItemId!=null && top.ptaiAguide.ptaiItemId)
 {AgPostMessage();}
}
}
...
var nLastKey_win4 = 0;var oLastKeyEvent = null;var gKeyDownTarget = null;function doKeyDown_win4(evt)
{
oLastKeyEvent = evt;var target = getEventTarget(evt);gKeyDownTarget = target;if (!isFModeLayout() && !PT_handleTabKeyForModalDialog(evt))
 return false;if (target && target.form && target.form.name != "win4")
 return findHandler("doKeyDown_" + target.form.name, evt);nLastKey_win4 = getKeyCode(evt);var bKeyUp = target && target.getAttribute('onkeyup') != null;if (isFModeLayout() && nLastKey_win4 == 13 && bKeyUp)
 evt.preventDefault();if (nLastKey_win4 != "\t".charCodeAt(0)) {

if (isGridNav(evt))
 ptGridObj_win4.doArrowNavGrid(evt);if (isTypeAheadEl(evt))
 ptTAObj_win4.KeyUpDown(evt);if(browserInfoObj2.isSafari && nLastKey_win4 == 27){
 var evt2=document.createEvent('Event'); evt2.initEvent('keypress',true,true); evt2.keyCode=evt.keyCode;  target.dispatchEvent(evt2); }

 return true;}

if (isCtrlKey(evt))
 return tryFocus(oFirstTab_win4);if (isAltKey(evt) || (isShiftKey(evt) && !isGridNav(evt)))
 return true;if (target == oLastTab_win4)
{
if (oFirstTB_win4)
 {
 if (!bTabOverTB_win4)
...
if (oFirstPg_win4)
 {
 if (!bTabOverPg_win4)
 return tryFocus(oFirstPg_win4); }
if (bTabOverNonPS_win4 || oFirstTB_win4 != null || oFirstPg_win4 != null)
 return tryFocus(oFirstTab_win4);return true;}
if (target == oLastTB_win4)
{
 if (oFirstPg_win4)
 {
 if (!bTabOverPg_win4)
 return tryFocus(oFirstPg_win4); }
 if (bTabOverNonPS_win4 || oFirstPg_win4 != null)
 return tryFocus(oFirstTab_win4); return true;}

if (target == oLastPg_win4)
{
 if (bTabOverNonPS_win4)
 return tryFocus(oFirstTab_win4); return true;}


if (nLastKey_win4 == "\t".charCodeAt(0) && isTypeAheadEl(evt))
{
 ptTAObj_win4.KeyUpDown(evt); return true;}

}

function getLastKey_win4()
{
var nTemp = nLastKey_win4;nLastKey_win4 = 0;return nTemp;}

function doKeyUp_win4(evt)
{
...
if (keyChar == "\r" && (!(isButton(target) || isRowAct(target)) || !isFModeLayout()) || key == 27)
 ...
 if (key == 27)
 {
 if (typeof (bDoModal_win4) != "undefined" && bDoModal_win4 && modalID != null) 
 doUpdateParent(document.win4, '#ICCancel');  else if (isFModeLayout() && bInModal)
 submitAction_win4(document.win4, '#ICCancel'); return; } 
...
if (!routeKeyEvent(evt))
 ...
 {
 if (bCtrl || altKey_win4.indexOf(keyChar) < 0)
 {
 ...
 keyChar = String.fromCharCode(key | 0x40); if (ctrlKey_win4.indexOf(keyChar) < 0)
 bNotMine = true; }
 ...
 {
 if (baseKey_win4.indexOf(keyChar) < 0)
 bNotMine = true; }
...
if (bNotMine) {
 ...
 ptTAObj_win4.GiveOptions(evt); return true;}


var code = getModifiers(evt) + keyChar;if (target && target.name)
 document.win4.ICFocus.value = target.name;if (code == "A8")
 if (!doDeleteKey_win4(target))
 return false;if (code == "A\xbf")
{
 if (window.FindString_win4
 && document.win4.ICFind
 && findScroll(target))
 {
 if (!FindString_win4(document.win4.ICFind))
 return false; }
else
 return false
}
...
{ 
 ...
 doUpdateParent(document.win4, '#ICReturn'); return; }
 ...
 submitAction_win4(document.win4, event.srcElement.id); else 
 submitAction_win4(document.win4, evt.target.id);}else
{
if (target && target.form)
 addchg_win4(target);if (code == 'A5'){
 submitAction_win4(document.win4, "#KEY" + code + operation, target.id);}else if(bAlt){
 if(key == 49) 
 setSaveText_win4('Saving...', true, false);  if(altKey_win4.indexOf(keyChar) >= 0 || keyChar == "2" || keyChar == "3" || keyChar == "4"){
 ...
 var url = "javascript:submitAction_win4(document.win4, \'#KEY" + code + "\')"; if (!saveWarning("", null, "", url))
 return false; }
 if( !ptCommonObj2.isSearchPage(target.form) && (code == "A\xbc" || code == "A\xbe" || code == "A\xde") && gActiveBtn && target.name == null)
 {
 document.win4.ICFocus.value = gActiveBtn.name
 submitAction_win4(document.win4, gActiveBtn.name); }
 else
 submitAction_win4(document.win4, "#KEY" + code);  }
}else{
 submitAction_win4(document.win4, "#KEY" + code);}
}
...
{
var obj = document.getElementById('win4divPAGEBAR');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win4divPSPANELTABS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win4divPSPANELTABLINKS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";}
...
{
return keyHandler_win4(keyCode, bIsAlt, bIsCtrl);}

function keyHandler_win4(keyCode, bIsAlt, bIsCtrl)
{
var keyChar = String.fromCharCode(keyCode);var code = "";if (bIsAlt)
{
 code = "A"; if (bIsCtrl || altKey_win4.indexOf(keyChar) < 0)
 return false;}
else if (bIsCtrl)
{
 code = "C"; keyChar = String.fromCharCode(keyCode | 0x40); if (ctrlKey_win4.indexOf(keyChar) < 0)
 return false;}
else
 return false;code += keyChar;if (code == "A8" || code == "A\xbf" || code == "A7")
 return false;if (target && target.form)
 addchg_win4(target);submitAction_win4(document.win4, "#KEY" + code);return true;}

function doDeleteKey_win4(obj)
{
if (!window.DeleteCheck2_win4)
 return false;var scroll = findScroll(obj);if (!scroll)
 return false;var bFocusable = true;if((obj.nodeName == 'BODY' || obj.id.length == 0) && preID_win4!= null ) {
 var temp = obj.ownerDocument.getElementById(preID_win4); if(typeof temp != 'undefined')
...
if (scroll.id.search(/^(.*)(\$scroll[im]?\$)(.*)$/,"$fdelete$") < 0)
 ...
 return DeleteCheck2_win4(buttonid);if(bFocusable) {
...
var row = bFocusable ? RegExp.$1 : RegExp.$3 - 1 ;if (buttonid.search(/^(.*)(\$fdelete\$)(.*)$/) < 0)
 ...
 return DeleteCheck2_win4(buttonid);return false;}

function doInsertKey_win4(obj)
{
if (!window.AddMultiple_win4)
 return false;var scroll = findScroll(obj);if (!scroll)
...
if (bHasInsertBtn)
{
 ...
 return AddMultiple_win4(document.win4.ICAddCount); if (scroll.id.match(/\$scrolli\$/))
 return true;}
return false;}

function findScroll(obj)
{
var temp = null;if(obj.nodeName == 'BODY' && preID_win4 != null ) {
 temp = obj.ownerDocument.getElementById(preID_win4); if(typeof temp != 'undefined')
 obj=temp;}
...
if (window.doKeyPress_win4 == null)
{
window.doKeyPress_win4 = function (evt)
 {
 oLastKeyEvent = evt; var event = evt ? evt : window.evt; var target = getEventTarget(event); if (target && target.form)
 addchg_win4(target); if (isFModeLayout() && target && (target.getAttribute('onkeyup') != null
 || target.getAttribute('onkeydown') != null))
 return; if (target && target.form && target.form.name != "win4")
 ...
 if (!isFModeLayout() && MOpopupObj_win4 && typeof(MOpopupObj_win4.bIs_shown) != "undefined" && MOpopupObj_win4.bIs_shown)
 return MOpopupObj_win4.StopAccess(event);  if (typeof (bDoModal_win4) != "undefined" && bDoModal_win4)
 ...
 document.win4.ICFocus.value = target.name;   if (key == 13) {
 if (isTypeAheadEl(event) && ptTAObj_win4.IsGrabHighlighted()) {
 return false; } 
 ...
 submitAction_win4(document.win4, "#KEY" + code); return false; }
}
...
var oFirstTab_win4 = null;var oLastTab_win4 = null;var oFirstTB_win4 = null;var oLastTB_win4 = null;var oFirstPg_win4 = null;var oLastPg_win4 = null;var nFirstTBIndex = 5000; var nFirstPgIndex = 10000;function checkTabIndex(obj)
{
if (obj.tabIndex && obj.tabIndex >= 0)
{
 if (obj.tabIndex < nFirstTBIndex)
 {
 if (oLastTab_win4 == null || obj.tabIndex > oLastTab_win4.tabIndex)
 oLastTab_win4 = obj; if (oFirstTab_win4 == null || obj.tabIndex < oFirstTab_win4.tabIndex)
 oFirstTab_win4 = obj; }
 else if (obj.tabIndex < nFirstPgIndex)
 {
 if (oLastTB_win4 == null || obj.tabIndex > oLastTB_win4.tabIndex)
 oLastTB_win4 = obj; if (oFirstTB_win4 == null || obj.tabIndex < oFirstTB_win4.tabIndex)
 oFirstTB_win4 = obj; }
 else
 {
 if (oLastPg_win4 == null || obj.tabIndex > oLastPg_win4.tabIndex)
 oLastPg_win4 = obj; if (oFirstPg_win4 == null || obj.tabIndex < oFirstPg_win4.tabIndex)
 oFirstPg_win4 = obj; }
}
}

function setEventHandlers_win4(sFirst, sLast, bMac)
{
var focus1, focus2;focus1 = function (evt) {doFocus_win4(this, true, true);};focus2 = function (evt) {doFocus_win4(this, false, true);};fchange = function(evt) {addchg_win4(this); };var i;if (sFirst!="")
{
...
var frm = document.win4;if (!frm) return;var frmlen = frm.length;objBeforeOrAfterFocusSave = null; var objBefore = null; var bAfter = 0; var b = navigator.userAgent.toLowerCase();var bIE = browserInfoObj2.isIE;var bIE8 = (bIE && b.indexOf("trident/4.0;") != -1) ? 1:0; for (i = 0; i < frmlen; ++i)
{
...
if (window.setKeyEventHandler_win4 == null)
{
 window.setKeyEventHandler_win4 = function ()
 {
 if (typeof(baseKey_win4) != "undefined")
 {
 document.onkeyup = doKeyUp_win4; if (baseKey_win4.indexOf("\r") >= 0 || baseKey_win4.indexOf("\x1b") >= 0)
 document.onkeypress = doKeyPress_win4; }
 document.onkeydown = doKeyDown_win4; }
}
...
function setFocus_win4(fldname, indx, oDoc0)
{
...
if (bRichTextEnabled == 0)
 ...
 oDoc = document; else
 oDoc = oDoc0;  var obj = null; if (oDoc.win4)
 {
 obj = oDoc.win4.elements[fldname]; if (!obj)
 obj = oDoc.win4[fldname]; }
...
{
 var currentNode = document.getElementById(document.win4.name+"div"+fldname); if(currentNode)
 {
 ...
 var element ; for (var i = 0; i < document.win4.elements.length; i++)
 {
 element = document.win4.elements[i]; if( element && element.tabIndex > 0 && element.type != "hidden" ) 
 { 
 try{
 tryFocus0New(element);  break;  }
 catch(e){
...
function tryFocus0New(obj)
{
 ...
 ptGridObj_win4.doScrollOnFocus(gn, obj); return false; }
return true;}
...
function setFocusSecPageMessage_win4()
{
var obj;if ((typeof bAccessibility_win4 != 'undefined') && (bAccessibility_win4)) 
 {
 obj = document.getElementById("MessageTitle_win4"); if (obj)
 return tryFocus(obj); }
...
function positionWAIT_win4(){
var waitobj = null;var savedobj = null;var objFrame = top.frames['TargetContent'];if (objFrame) {
 waitobj = objFrame.document.getElementById("WAIT_win4"); savedobj = objFrame.document.getElementById("SAVED_win4");}
else {
 waitobj = document.getElementById("WAIT_win4"); savedobj = document.getElementById("SAVED_win4");}
...
{
if (typeof ptGridObj_win4 == "undefined" || !ptGridObj_win4)
 return null;if (typeof gridList_win4 == "undefined" || typeof gridFieldList_win4 == "undefined")
...
for (var i = 0; i < gridList_win4.length; i++)
{
var gn = gridList_win4[i][0];var gc = document.getElementById('divgc'+gn);if (!gc || typeof gc == "undefined") continue;var gfields = gridFieldList_win4[i];for (var j = 0; j < gfields.length; j++)
{
...
{
var gn = this.isGridEl(evt);if (typeof ptGridObj_win4 != "undefined" && ptGridObj_win4 && gn != null)
{
...
function initTypeAheadEl(el) {
 ...
 icAction += "$" + nsuffix; }
 var serverScript = "ptTAObj_win4.oWin.pAction_win4(ptTAObj_win4.oDoc.win4,'" + icAction + "');"; el.obj = oWin.ptTAObj_win4.SetProperties(el, 0, form.ICTypeAheadID, serverScript, true, false, false, true, true, 500);  }
 ...
 return true; else if (typeof PFieldList_win4 != 'undefined' && PFieldList_win4) {
 var fname = removeOccursNum(el.id); for (var i = 0; i < PFieldList_win4.length; i++) {
 if (fname == PFieldList_win4[i][0] || el.id == PFieldList_win4[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 if (ptCommonObj2.isSearchSearchPage(el.form)) {
 if (typeof EFieldList_win4 != 'undefined' && EFieldList_win4) {
 for (var i = 0; i < EFieldList_win4.length; i++) {
 if (fname == EFieldList_win4[i][0] || el.id == EFieldList_win4[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 }
...
function UpdateBreadCrumbs() {
 ...
 bcUpdater.updateBreadCrumbs(document.win4); bcUpdater.removeRemoteData();  backNavigation.setCookie(); if (!isFModeLayout()) {
 updClassicHistory()
 }
...
function GetDomData(form, szTC, szActionName) {
 ...
 return;   var divHiddenFields = document.querySelector("#" + "win4" + "divPSHIDDENFIELDS")
 ...
 form.target = szTC; form.ICAction.value = szActionName; submitAction_win4(form, szActionName);}
...
function preSubmitWorkCenter_win4(arg1, arg2) {
 var isWorkCenter = top.document.getElementById('ptalPgltAreaFrame'); if(!isWorkCenter || !arg1) {return true; }

 if(parent.ptIframe) 
 return preSubmitCheckWorkCenter_win4(arg1,arg2,true);}

function preSubmitCheckWorkCenter_win4(arg1,arg2,bPtreplace) {
if(bPtreplace)
{
...
if (parent.ptIframe.isWorkCenterDirty()) { 
 ...
 tgtFrameForm.ICChanged.value='-1';  submitAction_win4(arg1, arg2); return;  }
 if(parent.ptIframe.saveWarningForWorkCenter(cancelFn,null ,1,arg1, arg2)) 
 return false; }
return true;}
function preSubmitProcessSpellcheck_win4(form, name) {
...
function preSubmitProcessCKEDITOR_win4(form, name) {
 if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField();}

function preSubmitProcess_win4(form, name) {
 if ((name.indexOf('$delete') > 0 || name.indexOf('$srt') > 0) && typeof (preID_win4) != 'undefined' && preID_win4 != null) 
 preID_win4 = null;  if (form == null) {
 form = document.forms['win4']; form.style.display = 'block'; }
 preSubmitProcessSpellcheck_win4(form, name); preSubmitProcessCKEDITOR_win4(form, name);     if (!isFModeLayout()) {
...
function aAction0_win4(form, name, params, bAjax, bPrompt)
{
if (isFModeLayout() && isPopupRequest(form, name)) return;if (typeof bSearchDialog_win4 != 'undefined' && !bDoModal_win4 && bSearchDialog_win4 && form.ICAction.value.indexOf("\x1b") >=0) 
 {
 processing_win4(0, 3000); return; }
...
if (typeof ptGridObj_win4 != 'undefined' && ptGridObj_win4)
 ptGridObj_win4.saveScrollPos();form.ICResubmit.value=nResubmit;form.ICAction.value=name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();bcUpdater.getAdvSrchLblFrmCriteria(name, form);var waitingObjList = null;if (loader != null && loader.GetWaitingICAction() != "") 
 waitingObjList = loader.GetWaitingObjectList();if (typeof bAjax != "undefined" && !bAjax && (typeof params == "undefined" || params == null || params == "") && (typeof popupObj_win4.isModalWidget == "undefined" || !popupObj_win4.isModalWidget))
 form.submit();else if (typeof params == "undefined" || !params)
 loader=new net2.ContentLoader(postUrl_win4,form,name,null,null,null,null,null,bAjax, bPrompt);else
 loader=new net2.ContentLoader(postUrl_win4,form,name,null,null,null,params,null,bAjax, bPrompt);if (waitingObjList != null)
 loader.WaitingObjQueue = waitingObjList;}

function pAction_win4(form, name, efieldname)
{
if(typeof CKEDITOR != 'undefined')UpdateEditorLinkedField();if ((loader != null) && (loader.GetInProcess())) return;form.ICAction.value = name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();if (typeof form.ICTypeAheadID != 'undefined' && form.ICTypeAheadID.value != '') {
 ...
 if (pm)
 pm.updateTypeaheadMsgEvent(name); var flag = form.ICActionPrompt.value; form.ICActionPrompt.value = 'false'; var ret = aAction_win4(form, name, "", true, 1); form.ICActionPrompt.value = flag; return ret;}
else {
 
 if (pm)
 pm.updatePromptMsgEvent(name); form.ICActionPrompt.value = 'true'; return aAction_win4(form, name, "", true, true);}
}

function mAction_win4(form, name, height, width, title, bModalElement, bSizeOnLoad)
{
 form.ICAction.value = name; form.ICXPos.value = ptCommonObj2.getScrollX(); form.ICYPos.value = ptCommonObj2.getScrollY(); aAction_win4(form, name, null, true, false);}





function changeKeyList_win4(obj, form, name, params)
{
 aAction_win4(form, name, params); var searchObj = document.getElementById('PT_SEARCH'); addClass(searchObj, 'side'); addClose(searchObj); SetSearchStatus(obj, form);}



function updateFromPrompt_win4(obj, form, name, params) {
 aAction_win4(form, name, params);}

function updateFromInPrompt_win4(obj, form, name, params) {
 var res = []; var arr1 = document.querySelectorAll(".ps_prompt-resultsgrid .ps_grid-body .psc_on"); for (var i = 0; i < arr1.length; i++)
 res.push(arr1[i].parentElement.parentElement.parentElement.querySelector(".ps_box-value").firstChild.nodeValue); var objHidden = document.getElementById("PT_WORK_PT_PROMPT_TEMP_IN"); objHidden.value = res.join(","); addchg_win4(objHidden); aAction_win4(form, name, params);}
...
function doModlessOnLoad_win4() {
if (window.name.length == 0)
 window.name = "win4";document.win4.target = window.name;}
...
function doModal(url, bPrompt, oParentWin, sCancelName, form, name, sPopupOptions, sResponseHTML) {
 ...
 window.modWin.onLoadExt_win4(); return; }
...
function setModWinParentPIA() {
 if (setModWinParent())
 document.win4.ICResubmit.value = 0;}

function doCancelMsg() {
 aAction_win4(document.win4, '#ICCancel');}
...
function doCancel(sCancelName) {
 ...
 document.win4.ICAction.value = "None"; else {
 ...
 var fm = win.document.win4; if (!fm) fm = window.document.win4; fm.ICAction.value = sCloseId; document.win4.ICAction.value = "None"; }
 }
 try {
 document.win4.target = winParent.name; if (document.win4.ICAction.value != "None") return; if (typeof window.bMFormSubmit != 'undefined')
 winParent.modWinClosed = null; winParent.nResubmit = 0;  if (window.document.win4.enctype.indexOf('multipart') != -1 && typeof window.bMFormSubmit != 'undefined' && !window.bMFormSubmit) {
 winParent.document.win4.ICAction.value = "None"; winParent.MTop().ptCommonObj2.hidePopupMask(); closeModal(modalID); }
 
 else if (sCloseId != "#ICCancel")
 aAction_win4(document.win4, sCloseId); else
 {
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + modalID); if ((typeof modObj != 'undefined') && modObj && modObj.bRCFModal && 
 (typeof(winParent.aAction_win4) == 'undefined'))
 {
 try
 {
 closeModal(modalID); }
 ...
 winParent.aAction_win4(document.win4, sCloseId); }
...
function onParentUnload_win4() {
 if ((typeof window.modWin != 'undefined') && window.modWin) closeModWin_win4();}

function doModalOnLoad_win4(bStartUp, bFileDetach, bNoParent) {
 ...
 && typeof winParent.document.win4 != 'undefined')
 winParent.processing_win4(0, 3000); resizeModalDialog_pt(modalID); } 
 } 
 ...
 if (window.document.win4.enctype.indexOf('multipart') != -1)
 window.bMFormSubmit = false;}

function doUpdateParent(form, name) {
 if (typeof winParent == 'undefined' || winParent == null ) {
 submitAction_win4(document.win4, name); return; }
 if (winParent.bDefer && name.indexOf("#ICCancel") != -1 && bPromptPage_win4) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win4.ICStateNum.value = document.win4.ICStateNum.value; winParent.aAction_win4(winParent.document.win4, '#ICCancel'); var cObj = winParent.document.getElementById("pt_modals"); if(cObj) cObj.style.display = "none"; } else {
 processing_win4(1, 3000); form.target = winParent.name; form.ICAction.value = name; if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField(); winParent.loader = null; if (pm)
  pm.updateParentMsgData(winParent,name);  winParent.aAction_win4(form, name); }
}

function doModalMFormSubmit_win4(form, name) {
 if (name.indexOf("#ICCancel") != -1) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win4.ICStateNum.value = document.win4.ICStateNum.value; winParent.aAction_win4(winParent.document.win4, '#ICCancel'); window.winParent.modWin = null; closeModal(window.modalID); return; }
 window.bMFormSubmit = true;  submitAction_win4(form, name); doModalMFormClose_win4();}

function doModalMFormClose_win4() {
 winParent.bUpLoadProgress = true; winParent.nResubmit = 1; winParent.document.win4.ICStateNum.value = document.win4.ICStateNum.value; winParent.loader = null; winParent.aAction_win4(winParent.document.win4, 'Resubmit'); if (typeof window.winParent != 'undefined' && window.winParent.winParent != null)
...
strCntr+="A";submitAction_win4(document.win4,strCntr);}





var bIsPagelet_win4 = "false", sPageletName_win4 = "";ResetGlyph_win4 = function() {
 if (typeof(sGlyphList_win4) !== "undefined" && sGlyphList_win4 !== "") {
 HideGlyph(sGlyphList_win4); }
}

setPageletInfoInCtxmenu_win4 = function(isPagelet,sPgltName) {
 bIsPagelet_win4 = (isPagelet === "false") ? "false" : "true"; sPageletName_win4 = (typeof(sPgltName) !== "undefined" && sPgltName && sPgltName !== "") ? sPgltName : "";}
...
};var ptTAObj_win4 = new PT_typeahead();function CreateRCMenu4PVG(sPvtGridName,nContext,nLeft,nTop,sActions,sPvgData,BulkActions)
{
...
strAction += "~"+BulkActions;var ch9=String.fromCharCode(9);var elemPVGAct = document.getElementById("ICPVGActMenu"); if (elemPVGAct)
 ...
 elemPVGData.value = sPvgData; submitAction_win4(document.win4,strAction);}

InvokePageAppCls = function(sServname,sData)
{
var sFormName = "win4";var form = document.forms[sFormName];if (typeof form.ICAPPCLSDATA != 'undefined') 
 form.ICAPPCLSDATA.value = sData;var sAppServname = "$APPCLS#"+sServname;submitAction_win4(document.win4,sAppServname);}
...
try {
 ...
 form.ICPVGData.value = sPvgData; var resPTxt = ""; var xmlDoc = null; var sLoader = new net2.ContentLoader(postUrl_win4, form, null, "post",function(){},
...
function doFolder(form, pid, fid) {
 ...
 objHidden.value = fid; addchg_win4(objHidden); submitAction_win4(form, pid);}
...
var bEventContinue= true;var tabSelect;var newTab = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI') 
 ...
 {
 document.win4.ICFocus.value = (tabSelect.firstElementChild || tabSelect.firstChild).id; currentTarget.onclick(); }
 break;  case 39: 
 case 40: 
...
var bReturn = false;var tabSelect = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI')
 ...
 document.win4.ICFocus.value = currentTarget.id.replace("_R", "_L"); else if (currentTarget.id.indexOf("ICTAB_SCROLL_L") == 0)
 document.win4.ICFocus.value = currentTarget.id.replace("_L", "_R"); else if (currentTarget.id.indexOf("ICTAB_HIDE") == 0)
 document.win4.ICFocus.value = currentTarget.id; currentTarget.onclick(); break; case 36: 
...
function InvokeFieldChange_win4(sFldName)
{
 submitAction_win4(document.win4, sFldName);}
function onRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML)
{

processing_win4(1, 3000);var sFormName = "win4";OpenRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML,sFormName);if (nOpenMode == "0")
...
{
 var changed = null; var objFrame = null;  if (document.win4)
 changed = checkFormChanged(document.win4, null);  if ((changed == null || !changed) && top.frames) {
...
{
 if (saveButton == null)
 return; var bZoom = isZoom(); setSaveText_win4('Saving...', true, false); bAutoSave = true; if (bZoom) {
 ...
 var icReturn = oWin.document.getElementById(TOOLS_RETURN); if ((icReturn != null) && (typeof oWin.document.win4.ICAutoSave != "undefined" )) {
 oWin.document.win4.ICAutoSave.value = '1'; icReturn.click(); } 
 }
 }
 
 if (!bZoom) {
 submitAction_win4(document.win4, saveButton); } else {
 
 setTimeout(function(){submitAction_win4(document.win4,saveButton)}, 5000);  }
 }
...
function DisplayNUIRCMenu_win4(FieldName,nLeft,nTop,sMenu) {
 ...
 var mDivObj = document.getElementById('win4' + 'div' + FieldName + '$divpop'); if (!mDivObj)
 ...
 processing_win4(0, 3000); resetMenu(mDivObj); var sClass = mDivObj.getAttribute("class"); sClass = sClass.replace(" psc_hidden", ""); mDivObj.setAttribute("class", sClass); if (typeof(nLeft) != 'undefined' && typeof(nTop) != 'undefined')
 {
...
function DisplayNUIRCMenu(FieldName,sMenu,nLeft,nTop) {
 DisplayNUIRCMenu_win4(FieldName,nLeft,nTop,sMenu);}

InvokeAppClsService_win4 = function(sFldName,sService,sData)
{
var nPos = sFldName.lastIndexOf("$");var szBase = sFldName;if (nPos != -1)
 ...
 szBase = sFldName+"$APPCLS#"+sService;var sFormName = "win4";var form = document.forms[sFormName];if (typeof form.ICAPPCLSDATA != 'undefined') 
 form.ICAPPCLSDATA.value = sData;submitAction_win4(document.win4,szBase);}
