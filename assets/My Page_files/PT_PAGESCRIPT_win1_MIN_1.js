...
{
var saveobj = document.getElementById("SAVED_win1");if (saveobj)
 {
 if (txt == "") txt = 'Saving...';  document.getElementById("ptStatusText_win1").innerHTML = txt; showhide(saveobj, true); document.getElementById("saveWait_win1").style.display = ""; }
}

function processing_win1(opt,waittime)
{
var waitobj = document.getElementById("WAIT_win1");var saveobj = document.getElementById("SAVED_win1");var saveWaitObj = document.getElementById("saveWait_win1");updateWindowTitleFromFakeBC(); if (opt == 0)
{
...
if (opt == 2)
{
 showhide(waitobj, false); setSaveText_win1('Saved'); if (saveWaitObj)
 saveWaitObj.style.display = "none"; bTimer = setTimeout("processing_win1(0)",waittime);  if ((typeof top.ptaiAguide=='object') && top.ptaiAguide.ptaiItemId!=null && top.ptaiAguide.ptaiItemId)
 {AgPostMessage();}
}
}
...
var nLastKey_win1 = 0;var oLastKeyEvent = null;var gKeyDownTarget = null;function doKeyDown_win1(evt)
{
oLastKeyEvent = evt;var target = getEventTarget(evt);gKeyDownTarget = target;if (!isFModeLayout() && !PT_handleTabKeyForModalDialog(evt))
 return false;if (target && target.form && target.form.name != "win1")
 return findHandler("doKeyDown_" + target.form.name, evt);nLastKey_win1 = getKeyCode(evt);var bKeyUp = target && target.getAttribute('onkeyup') != null;if (isFModeLayout() && nLastKey_win1 == 13 && bKeyUp)
 evt.preventDefault();if (nLastKey_win1 != "\t".charCodeAt(0)) {

if (isGridNav(evt))
 ptGridObj_win1.doArrowNavGrid(evt);if (isTypeAheadEl(evt))
 ptTAObj_win1.KeyUpDown(evt);if(browserInfoObj2.isSafari && nLastKey_win1 == 27){
 var evt2=document.createEvent('Event'); evt2.initEvent('keypress',true,true); evt2.keyCode=evt.keyCode;  target.dispatchEvent(evt2); }

 return true;}

if (isCtrlKey(evt))
 return tryFocus(oFirstTab_win1);if (isAltKey(evt) || (isShiftKey(evt) && !isGridNav(evt)))
 return true;if (target == oLastTab_win1)
{
if (oFirstTB_win1)
 {
 if (!bTabOverTB_win1)
...
if (bTabOverNonPS_win1 || oFirstTB_win1 != null || oFirstPg_win1 != null)
 return tryFocus(oFirstTab_win1);return true;}
...
{
 ptTAObj_win1.KeyUpDown(evt); return true;}

}

function getLastKey_win1()
{
var nTemp = nLastKey_win1;nLastKey_win1 = 0;return nTemp;}
...
if (keyChar == "\r" && (!(isButton(target) || isRowAct(target)) || !isFModeLayout()) || key == 27)
 ...
 if (key == 27)
 {
 if (typeof (bDoModal_win1) != "undefined" && bDoModal_win1 && modalID != null) 
 doUpdateParent(document.win1, '#ICCancel');  else if (isFModeLayout() && bInModal)
 submitAction_win1(document.win1, '#ICCancel'); return; } 
...
if (!routeKeyEvent(evt))
 ...
 keyChar = String.fromCharCode(key | 0x40); if (ctrlKey_win1.indexOf(keyChar) < 0)
 bNotMine = true; }
...
if (bNotMine) {
 ...
 ptTAObj_win1.GiveOptions(evt); return true;}


var code = getModifiers(evt) + keyChar;if (target && target.name)
 document.win1.ICFocus.value = target.name;if (code == "A8")
...
{ 
 ...
 doUpdateParent(document.win1, '#ICReturn'); return; }
 ...
 submitAction_win1(document.win1, event.srcElement.id); else 
 submitAction_win1(document.win1, evt.target.id);}else
{
if (target && target.form)
 addchg_win1(target);if (code == 'A5'){
 submitAction_win1(document.win1, "#KEY" + code + operation, target.id);}else if(bAlt){
 if(key == 49) 
 setSaveText_win1('Saving...', true, false);  if(altKey_win1.indexOf(keyChar) >= 0 || keyChar == "2" || keyChar == "3" || keyChar == "4"){
 ...
 var url = "javascript:submitAction_win1(document.win1, \'#KEY" + code + "\')"; if (!saveWarning("", null, "", url))
 return false; }
 if( !ptCommonObj2.isSearchPage(target.form) && (code == "A\xbc" || code == "A\xbe" || code == "A\xde") && gActiveBtn && target.name == null)
 {
 document.win1.ICFocus.value = gActiveBtn.name
 submitAction_win1(document.win1, gActiveBtn.name); }
 else
...
{
var obj = document.getElementById('win1divPAGEBAR');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win1divPSPANELTABS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win1divPSPANELTABLINKS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";}
...
{
 code = "A"; if (bIsCtrl || altKey_win1.indexOf(keyChar) < 0)
 return false;}
else if (bIsCtrl)
{
 code = "C"; keyChar = String.fromCharCode(keyCode | 0x40); if (ctrlKey_win1.indexOf(keyChar) < 0)
 return false;}
else
 return false;code += keyChar;if (code == "A8" || code == "A\xbf" || code == "A7")
 return false;if (target && target.form)
 addchg_win1(target);submitAction_win1(document.win1, "#KEY" + code);return true;}

function doDeleteKey_win1(obj)
{
if (!window.DeleteCheck2_win1)
 return false;var scroll = findScroll(obj);if (!scroll)
 return false;var bFocusable = true;if((obj.nodeName == 'BODY' || obj.id.length == 0) && preID_win1!= null ) {
 var temp = obj.ownerDocument.getElementById(preID_win1); if(typeof temp != 'undefined')
...
if (scroll.id.search(/^(.*)(\$scroll[im]?\$)(.*)$/,"$fdelete$") < 0)
 ...
 return DeleteCheck2_win1(buttonid);if(bFocusable) {
...
var row = bFocusable ? RegExp.$1 : RegExp.$3 - 1 ;if (buttonid.search(/^(.*)(\$fdelete\$)(.*)$/) < 0)
 ...
 return DeleteCheck2_win1(buttonid);return false;}
...
if (bHasInsertBtn)
{
 ...
 return AddMultiple_win1(document.win1.ICAddCount); if (scroll.id.match(/\$scrolli\$/))
 return true;}
return false;}

function findScroll(obj)
{
var temp = null;if(obj.nodeName == 'BODY' && preID_win1 != null ) {
 temp = obj.ownerDocument.getElementById(preID_win1); if(typeof temp != 'undefined')
 obj=temp;}
...
window.doKeyPress_win1 = function (evt)
 ...
 addchg_win1(target); if (isFModeLayout() && target && (target.getAttribute('onkeyup') != null
 || target.getAttribute('onkeydown') != null))
 return; if (target && target.form && target.form.name != "win1")
 ...
 if (!isFModeLayout() && MOpopupObj_win1 && typeof(MOpopupObj_win1.bIs_shown) != "undefined" && MOpopupObj_win1.bIs_shown)
 return MOpopupObj_win1.StopAccess(event);  if (typeof (bDoModal_win1) != "undefined" && bDoModal_win1)
 ...
 document.win1.ICFocus.value = target.name;   if (key == 13) {
 if (isTypeAheadEl(event) && ptTAObj_win1.IsGrabHighlighted()) {
 return false; } 
 ...
 submitAction_win1(document.win1, "#KEY" + code); return false; }
}
...
var oFirstTab_win1 = null;var oLastTab_win1 = null;var oFirstTB_win1 = null;var oLastTB_win1 = null;var oFirstPg_win1 = null;var oLastPg_win1 = null;var nFirstTBIndex = 5000; var nFirstPgIndex = 10000;function checkTabIndex(obj)
{
if (obj.tabIndex && obj.tabIndex >= 0)
{
 ...
 oLastTab_win1 = obj; if (oFirstTab_win1 == null || obj.tabIndex < oFirstTab_win1.tabIndex)
 oFirstTab_win1 = obj; }
 else if (obj.tabIndex < nFirstPgIndex)
 {
 if (oLastTB_win1 == null || obj.tabIndex > oLastTB_win1.tabIndex)
 oLastTB_win1 = obj; if (oFirstTB_win1 == null || obj.tabIndex < oFirstTB_win1.tabIndex)
 oFirstTB_win1 = obj; }
 else
 {
 if (oLastPg_win1 == null || obj.tabIndex > oLastPg_win1.tabIndex)
 oLastPg_win1 = obj; if (oFirstPg_win1 == null || obj.tabIndex < oFirstPg_win1.tabIndex)
 oFirstPg_win1 = obj; }
}
}
...
{
var focus1, focus2;focus1 = function (evt) {doFocus_win1(this, true, true);};focus2 = function (evt) {doFocus_win1(this, false, true);};fchange = function(evt) {addchg_win1(this); };var i;if (sFirst!="")
{
...
var frm = document.win1;if (!frm) return;var frmlen = frm.length;objBeforeOrAfterFocusSave = null; var objBefore = null; var bAfter = 0; var b = navigator.userAgent.toLowerCase();var bIE = browserInfoObj2.isIE;var bIE8 = (bIE && b.indexOf("trident/4.0;") != -1) ? 1:0; for (i = 0; i < frmlen; ++i)
{
...
if (window.setKeyEventHandler_win1 == null)
{
 ...
 {
 document.onkeyup = doKeyUp_win1; if (baseKey_win1.indexOf("\r") >= 0 || baseKey_win1.indexOf("\x1b") >= 0)
...
if (bRichTextEnabled == 0)
 ...
 oDoc = document; else
 oDoc = oDoc0;  var obj = null; if (oDoc.win1)
 {
 obj = oDoc.win1.elements[fldname]; if (!obj)
...
{
 var currentNode = document.getElementById(document.win1.name+"div"+fldname); if(currentNode)
 {
 ...
 var element ; for (var i = 0; i < document.win1.elements.length; i++)
 {
 element = document.win1.elements[i]; if( element && element.tabIndex > 0 && element.type != "hidden" ) 
 { 
 try{
 tryFocus0New(element);  break;  }
 catch(e){
...
function tryFocus0New(obj)
{
 ...
 ptGridObj_win1.doScrollOnFocus(gn, obj); return false; }
return true;}
...
{
var obj;if ((typeof bAccessibility_win1 != 'undefined') && (bAccessibility_win1)) 
 {
 obj = document.getElementById("MessageTitle_win1"); if (obj)
 return tryFocus(obj); }
...
var waitobj = null;var savedobj = null;var objFrame = top.frames['TargetContent'];if (objFrame) {
 waitobj = objFrame.document.getElementById("WAIT_win1"); savedobj = objFrame.document.getElementById("SAVED_win1");}
else {
 waitobj = document.getElementById("WAIT_win1"); savedobj = document.getElementById("SAVED_win1");}
...
if (typeof ptGridObj_win1 == "undefined" || !ptGridObj_win1)
 return null;if (typeof gridList_win1 == "undefined" || typeof gridFieldList_win1 == "undefined")
...
{
var gn = gridList_win1[i][0];var gc = document.getElementById('divgc'+gn);if (!gc || typeof gc == "undefined") continue;var gfields = gridFieldList_win1[i];for (var j = 0; j < gfields.length; j++)
{
...
{
var gn = this.isGridEl(evt);if (typeof ptGridObj_win1 != "undefined" && ptGridObj_win1 && gn != null)
{
...
function initTypeAheadEl(el) {
 ...
 icAction += "$" + nsuffix; }
 var serverScript = "ptTAObj_win1.oWin.pAction_win1(ptTAObj_win1.oDoc.win1,'" + icAction + "');"; el.obj = oWin.ptTAObj_win1.SetProperties(el, 0, form.ICTypeAheadID, serverScript, true, false, false, true, true, 500);  }
 ...
 return true; else if (typeof PFieldList_win1 != 'undefined' && PFieldList_win1) {
 var fname = removeOccursNum(el.id); for (var i = 0; i < PFieldList_win1.length; i++) {
 if (fname == PFieldList_win1[i][0] || el.id == PFieldList_win1[i][0]) {
 ...
 if (fname == EFieldList_win1[i][0] || el.id == EFieldList_win1[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 }
...
function UpdateBreadCrumbs() {
 ...
 bcUpdater.updateBreadCrumbs(document.win1); bcUpdater.removeRemoteData();  backNavigation.setCookie(); if (!isFModeLayout()) {
 updClassicHistory()
 }
...
function GetDomData(form, szTC, szActionName) {
 ...
 return;   var divHiddenFields = document.querySelector("#" + "win1" + "divPSHIDDENFIELDS")
 ...
 form.target = szTC; form.ICAction.value = szActionName; submitAction_win1(form, szActionName);}
...
function preSubmitWorkCenter_win1(arg1, arg2) {
 ...
 if(parent.ptIframe) 
 return preSubmitCheckWorkCenter_win1(arg1,arg2,true);}

function preSubmitCheckWorkCenter_win1(arg1,arg2,bPtreplace) {
if(bPtreplace)
{
...
if (parent.ptIframe.isWorkCenterDirty()) { 
 ...
 tgtFrameForm.ICChanged.value='-1';  submitAction_win1(arg1, arg2); return;  }
...
function preSubmitProcess_win1(form, name) {
 if ((name.indexOf('$delete') > 0 || name.indexOf('$srt') > 0) && typeof (preID_win1) != 'undefined' && preID_win1 != null) 
 preID_win1 = null;  if (form == null) {
 form = document.forms['win1']; form.style.display = 'block'; }
 preSubmitProcessSpellcheck_win1(form, name); preSubmitProcessCKEDITOR_win1(form, name);     if (!isFModeLayout()) {
...
{
if (isFModeLayout() && isPopupRequest(form, name)) return;if (typeof bSearchDialog_win1 != 'undefined' && !bDoModal_win1 && bSearchDialog_win1 && form.ICAction.value.indexOf("\x1b") >=0) 
 {
 processing_win1(0, 3000); return; }
...
if (typeof ptGridObj_win1 != 'undefined' && ptGridObj_win1)
 ptGridObj_win1.saveScrollPos();form.ICResubmit.value=nResubmit;form.ICAction.value=name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();bcUpdater.getAdvSrchLblFrmCriteria(name, form);var waitingObjList = null;if (loader != null && loader.GetWaitingICAction() != "") 
 waitingObjList = loader.GetWaitingObjectList();if (typeof bAjax != "undefined" && !bAjax && (typeof params == "undefined" || params == null || params == "") && (typeof popupObj_win1.isModalWidget == "undefined" || !popupObj_win1.isModalWidget))
 form.submit();else if (typeof params == "undefined" || !params)
 loader=new net2.ContentLoader(postUrl_win1,form,name,null,null,null,null,null,bAjax, bPrompt);else
 loader=new net2.ContentLoader(postUrl_win1,form,name,null,null,null,params,null,bAjax, bPrompt);if (waitingObjList != null)
...
if(typeof CKEDITOR != 'undefined')UpdateEditorLinkedField();if ((loader != null) && (loader.GetInProcess())) return;form.ICAction.value = name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();if (typeof form.ICTypeAheadID != 'undefined' && form.ICTypeAheadID.value != '') {
 ...
 if (pm)
 pm.updateTypeaheadMsgEvent(name); var flag = form.ICActionPrompt.value; form.ICActionPrompt.value = 'false'; var ret = aAction_win1(form, name, "", true, 1); form.ICActionPrompt.value = flag; return ret;}
else {
 
 if (pm)
 pm.updatePromptMsgEvent(name); form.ICActionPrompt.value = 'true'; return aAction_win1(form, name, "", true, true);}
}

function mAction_win1(form, name, height, width, title, bModalElement, bSizeOnLoad)
{
 form.ICAction.value = name; form.ICXPos.value = ptCommonObj2.getScrollX(); form.ICYPos.value = ptCommonObj2.getScrollY(); aAction_win1(form, name, null, true, false);}





function changeKeyList_win1(obj, form, name, params)
{
 aAction_win1(form, name, params); var searchObj = document.getElementById('PT_SEARCH'); addClass(searchObj, 'side'); addClose(searchObj); SetSearchStatus(obj, form);}



function updateFromPrompt_win1(obj, form, name, params) {
 aAction_win1(form, name, params);}

function updateFromInPrompt_win1(obj, form, name, params) {
 var res = []; var arr1 = document.querySelectorAll(".ps_prompt-resultsgrid .ps_grid-body .psc_on"); for (var i = 0; i < arr1.length; i++)
 res.push(arr1[i].parentElement.parentElement.parentElement.querySelector(".ps_box-value").firstChild.nodeValue); var objHidden = document.getElementById("PT_WORK_PT_PROMPT_TEMP_IN"); objHidden.value = res.join(","); addchg_win1(objHidden); aAction_win1(form, name, params);}
...
if (window.name.length == 0)
 window.name = "win1";document.win1.target = window.name;}
...
function doModal(url, bPrompt, oParentWin, sCancelName, form, name, sPopupOptions, sResponseHTML) {
 ...
 window.modWin.onLoadExt_win1(); return; }
...
function doCancel(sCancelName) {
 ...
 document.win1.ICAction.value = "None"; else {
 ...
 var fm = win.document.win1; if (!fm) fm = window.document.win1; fm.ICAction.value = sCloseId; document.win1.ICAction.value = "None"; }
 }
 try {
 document.win1.target = winParent.name; if (document.win1.ICAction.value != "None") return; if (typeof window.bMFormSubmit != 'undefined')
 winParent.modWinClosed = null; winParent.nResubmit = 0;  if (window.document.win1.enctype.indexOf('multipart') != -1 && typeof window.bMFormSubmit != 'undefined' && !window.bMFormSubmit) {
 winParent.document.win1.ICAction.value = "None"; winParent.MTop().ptCommonObj2.hidePopupMask(); closeModal(modalID); }
 
 else if (sCloseId != "#ICCancel")
 aAction_win1(document.win1, sCloseId); else
 {
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + modalID); if ((typeof modObj != 'undefined') && modObj && modObj.bRCFModal && 
 (typeof(winParent.aAction_win1) == 'undefined'))
 {
 try
 {
 closeModal(modalID); }
...
function closeModWin_win1() {
 ...
 if (window.modWin) {
 try {
 window.modWin.document.win1.ICAction.value = '#ICCancel'; closeModal(window.modWin.modalID); }
 catch (err) {
 }
 }
 window.modWin = null;}
...
function doModalOnLoad_win1(bStartUp, bFileDetach, bNoParent) {
 ...
 winParent.processing_win1(0, 3000); resizeModalDialog_pt(modalID); } 
 } 
...
function doUpdateParent(form, name) {
 ...
 submitAction_win1(document.win1, name); return; }
 if (winParent.bDefer && name.indexOf("#ICCancel") != -1 && bPromptPage_win1) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win1.ICStateNum.value = document.win1.ICStateNum.value; winParent.aAction_win1(winParent.document.win1, '#ICCancel'); var cObj = winParent.document.getElementById("pt_modals"); if(cObj) cObj.style.display = "none"; } else {
 processing_win1(1, 3000); form.target = winParent.name; form.ICAction.value = name; if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField(); winParent.loader = null; if (pm)
  pm.updateParentMsgData(winParent,name);  winParent.aAction_win1(form, name); }
}

function doModalMFormSubmit_win1(form, name) {
 if (name.indexOf("#ICCancel") != -1) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win1.ICStateNum.value = document.win1.ICStateNum.value; winParent.aAction_win1(winParent.document.win1, '#ICCancel'); window.winParent.modWin = null; closeModal(window.modalID); return; }
 window.bMFormSubmit = true;  submitAction_win1(form, name); doModalMFormClose_win1();}

function doModalMFormClose_win1() {
 winParent.bUpLoadProgress = true; winParent.nResubmit = 1; winParent.document.win1.ICStateNum.value = document.win1.ICStateNum.value; winParent.loader = null; winParent.aAction_win1(winParent.document.win1, 'Resubmit'); if (typeof window.winParent != 'undefined' && window.winParent.winParent != null)
...
strCntr+="A";submitAction_win1(document.win1,strCntr);}





var bIsPagelet_win1 = "false", sPageletName_win1 = "";ResetGlyph_win1 = function() {
...
setPageletInfoInCtxmenu_win1 = function(isPagelet,sPgltName) {
 bIsPagelet_win1 = (isPagelet === "false") ? "false" : "true"; sPageletName_win1 = (typeof(sPgltName) !== "undefined" && sPgltName && sPgltName !== "") ? sPgltName : "";}
...
};var ptTAObj_win1 = new PT_typeahead();function CreateRCMenu4PVG(sPvtGridName,nContext,nLeft,nTop,sActions,sPvgData,BulkActions)
{
...
strAction += "~"+BulkActions;var ch9=String.fromCharCode(9);var elemPVGAct = document.getElementById("ICPVGActMenu"); if (elemPVGAct)
 ...
 elemPVGData.value = sPvgData; submitAction_win1(document.win1,strAction);}

InvokePageAppCls = function(sServname,sData)
{
var sFormName = "win1";var form = document.forms[sFormName];if (typeof form.ICAPPCLSDATA != 'undefined') 
 form.ICAPPCLSDATA.value = sData;var sAppServname = "$APPCLS#"+sServname;submitAction_win1(document.win1,sAppServname);}
...
try {
 ...
 form.ICPVGData.value = sPvgData; var resPTxt = ""; var xmlDoc = null; var sLoader = new net2.ContentLoader(postUrl_win1, form, null, "post",function(){},
...
function doFolder(form, pid, fid) {
 ...
 objHidden.value = fid; addchg_win1(objHidden); submitAction_win1(form, pid);}
...
var bEventContinue= true;var tabSelect;var newTab = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI') 
 ...
 {
 document.win1.ICFocus.value = (tabSelect.firstElementChild || tabSelect.firstChild).id; currentTarget.onclick(); }
 break;  case 39: 
 case 40: 
...
var bReturn = false;var tabSelect = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI')
 ...
 document.win1.ICFocus.value = currentTarget.id.replace("_R", "_L"); else if (currentTarget.id.indexOf("ICTAB_SCROLL_L") == 0)
 document.win1.ICFocus.value = currentTarget.id.replace("_L", "_R"); else if (currentTarget.id.indexOf("ICTAB_HIDE") == 0)
 document.win1.ICFocus.value = currentTarget.id; currentTarget.onclick(); break; case 36: 
...
function InvokeFieldChange_win1(sFldName)
{
 submitAction_win1(document.win1, sFldName);}
function onRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML)
{

processing_win1(1, 3000);var sFormName = "win1";OpenRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML,sFormName);if (nOpenMode == "0")
...
{
 var changed = null; var objFrame = null;  if (document.win1)
 changed = checkFormChanged(document.win1, null);  if ((changed == null || !changed) && top.frames) {
...
{
 if (saveButton == null)
 return; var bZoom = isZoom(); setSaveText_win1('Saving...', true, false); bAutoSave = true; if (bZoom) {
 ...
 var icReturn = oWin.document.getElementById(TOOLS_RETURN); if ((icReturn != null) && (typeof oWin.document.win1.ICAutoSave != "undefined" )) {
 oWin.document.win1.ICAutoSave.value = '1'; icReturn.click(); } 
 }
 }
 
 if (!bZoom) {
 submitAction_win1(document.win1, saveButton); } else {
 
 setTimeout(function(){submitAction_win1(document.win1,saveButton)}, 5000);  }
 }
...
function DisplayNUIRCMenu_win1(FieldName,nLeft,nTop,sMenu) {
 ...
 var mDivObj = document.getElementById('win1' + 'div' + FieldName + '$divpop'); if (!mDivObj)
 ...
 processing_win1(0, 3000); resetMenu(mDivObj); var sClass = mDivObj.getAttribute("class"); sClass = sClass.replace(" psc_hidden", ""); mDivObj.setAttribute("class", sClass); if (typeof(nLeft) != 'undefined' && typeof(nTop) != 'undefined')
 {
...
function DisplayNUIRCMenu(FieldName,sMenu,nLeft,nTop) {
 DisplayNUIRCMenu_win1(FieldName,nLeft,nTop,sMenu);}

InvokeAppClsService_win1 = function(sFldName,sService,sData)
{
var nPos = sFldName.lastIndexOf("$");var szBase = sFldName;if (nPos != -1)
 ...
 szBase = sFldName+"$APPCLS#"+sService;var sFormName = "win1";var form = document.forms[sFormName];if (typeof form.ICAPPCLSDATA != 'undefined') 
 form.ICAPPCLSDATA.value = sData;submitAction_win1(document.win1,szBase);}
