...
{
if (typeof ptGridObj_win1.gridArr=="undefined") return;for (var i = 0; i < ptGridObj_win1.gridArr.length; i++)
{
var gid = ptGridObj_win1.gridArr[i][0]; var brobj=document.getElementById('divgbr'+gid);if (brobj)
{
ptGridObj_win1.gridArr[i][1] = brobj.scrollLeft;ptGridObj_win1.gridArr[i][2] = brobj.scrollTop;}
}
},
...
getScrollPos:function(gridID)
{
 ...
 return {x:0, y:0};  if (typeof ptGridObj_win1.gridArr=="undefined") 
 return {x:0, y:0};  for (var i = 0; i < ptGridObj_win1.gridArr.length; i++)
 {
 var gid = ptGridObj_win1.gridArr[i][0];  if (gid.length == gridID.length && gid.indexOf(gridID) == 0)
...
{
if (typeof ptGridObj_win1.gridArr=="undefined") return;ptGridObj_win1.bRestore = true;for (var i = 0; i < ptGridObj_win1.gridArr.length; i++)
{
var gid = ptGridObj_win1.gridArr[i][0]; var brobj=document.getElementById('divgbr'+gid);var hrobj=document.getElementById('divghr'+gid);var blobj=document.getElementById('divgbl'+gid);if (brobj)
{
brobj.scrollLeft=ptGridObj_win1.gridArr[i][1];brobj.scrollTop=ptGridObj_win1.gridArr[i][2];}
if (hrobj)
 hrobj.scrollLeft = ptGridObj_win1.gridArr[i][1];if (blobj)
...
onSort:function(e)
{
if (e && e.button == 2) return; var obj = getEventTarget(e);ptGridObj_win1.doSort(obj.id);},
doSort:function(id)
{
var tmp = id.split("$");var gid_s = tmp[0];var gocc_s = tmp[1].split("#")[0];var gid = gid_s+'$'+gocc_s;var bSortable = false;for (var i = 0; i < gridList_win1.length; i++)
 {
 var ginfo = gridList_win1[i]; if (gid && gid != ginfo[0])
 continue;  var gheaders = gridHeaderList_win1[i];  if (!gheaders || typeof gheaders == "undefined")
...
trgelem[3] = 2;else
trgelem[3] = 1;cols[col] = trgelem.join("|"); cmap.value = cols.join("#"); submitAction_win1(document.win1,sId);},
...
if (!gridList_win1 || typeof gridList_win1 == "undefined" || !gridHeaderList_win1 || typeof gridHeaderList_win1 == "undefined")
 return;for (var i = 0; i < gridList_win1.length; i++)
 {
 var ginfo = gridList_win1[i]; if ( ginfo[4] && ginfo[4] != 1) continue; var gheaders = gridHeaderList_win1[i];  if (!gheaders || typeof gheaders == "undefined")
...
initGrid: function(gid) {
 if (typeof gid == "undefined" || !gid || typeof gridList_win1 == "undefined") 
 return; var objHC = document.getElementById('divghrc' + gid); if (!objHC || (typeof objHC.bSet != 'undefined' && objHC.bSet)) 
 return;  for (var i = 0; i < gridList_win1.length; i++) {
 var ginfo = gridList_win1[i]; if (ginfo[4] && ginfo[4] != 1) continue; if (gid && gid != ginfo[0])
 continue; var nStart = gridList_win1[i][1]; var nStop = gridList_win1[i][2]; var gheaders = gridHeaderList_win1[i]; var gfields = gridFieldList_win1[i]; if (!gheaders || typeof gheaders == "undefined" || !gfields || typeof gfields == "undefined")
 ...
 var sObj = document.getElementById(fn); if (!sObj || typeof sObj == "undefined") continue; ptEvent.add(sObj, "mouseover", ptGridObj_win1.setHandCursor); ptEvent.add(sObj, "mousedown", ptGridObj_win1.onSort); continue; } 
 else {
 ...
 ptEvent.add(hObj, "mouseover", ptGridResizeObj_win1.setResizeCursor); ptEvent.add(hObj, "mousedown", ptGridResizeObj_win1.TDselDown); }
 if (!browserInfoObj2.isIE) {
 var oTDsel = document.getElementById('pt_dragResize'); ptEvent.add(oTDsel, "mouseover", ptGridResizeObj_win1.setResizeCursor); ptEvent.add(oTDsel, "mouseup", ptGridResizeObj_win1.TDselUp); ptEvent.add(oTDsel, "mousemove", ptGridResizeObj_win1.TDselMove); ptEvent.add(oTDsel, "dragstart", ptGridResizeObj_win1.clearevt); ptEvent.add(oTDsel, "selectstart", ptGridResizeObj_win1.clearevt); }
 if (this.IsFreezeCol(cmap, src[1]) && gridList_win1[i][3] == 1) {
 if (bSortable) {
 var sObj = document.getElementById(fn); if (!sObj || typeof sObj == "undefined") continue; ptEvent.add(sObj, "mouseover", ptGridObj_win1.setHandCursor); ptEvent.add(sObj, "mousedown", ptGridObj_win1.onSort); }
 continue; }
 ...
 if ((this.IsFreezeCol(cmap, src[1]) && this.getNumOfFreezeCol(cmap) > 1) ||
 (this.IsFlowCol(cmap, src[1]) && (gheaders.length - this.getNumOfFreezeCol(cmap)) > 1)) {
 var osrc = document.getElementById(fn); ptEvent.add(osrc, "mouseover", ptGridObj_win1.setMoveCursor); ptGridDDObj_win1.addSource(osrc, false); }
 else {
 ...
 if (ptGridDDObj_win1 != null)
 ptGridDDObj_win1.init(); objHC.bSet = true;},


initGriddd854:function(){
if (ptConsole2.isActive() && bPerf)
 ptConsole2.append((new Date()).valueOf() + " Grid init start");if (typeof gridList_win1 == "undefined" || typeof gridHeaderList_win1 == "undefined" || typeof gridFieldList_win1 == "undefined" || typeof gridFieldList_win1[0] == "undefined")
 return;ptGridDDObj_win1 = new PT_gridDD();ptGridResizeObj_win1 = new PT_gridResize();var bArr=false;if (!ptGridObj_win1.gridArr)
{
 ptGridObj_win1.gridArr=new Array(); bArr=true; ptGridObj_win1.bRestore = false;}

if (!browserInfoObj2.isIE)
 this.initSetCapture();for (var i = 0; i < gridList_win1.length; i++)
 {
 var ginfo = gridList_win1[i]; if (bArr) 
...
ptGridObj_win1.adjustColumnWidthHeight();if (ptConsole2.isActive() && bPerf)
 ptConsole2.append((new Date()).valueOf() + " Grid init end");},

resizeItem:function(srcId,val)
{
var src = srcId.split("#");var tmp = src[0].split("$");var gid_s = tmp[0];var gocc_s = tmp[1];var cmap = document.getElementById(gid_s+"$hnewpers$0");this.setSize(cmap,src[1],val);ptGridObj_win1.saveScrollPos();submitAction_win1(document.win1,gid_s+"$hnewpers$0");},
...
nScrollLeft = new Number(ogbr_div.scrollLeft);ptGridObj_win1.saveScrollPos();submitAction_win1(document.win1,gid_s+"$hnewpers$0");},
doArrowNavGrid:function(evt)
{
 evt= (evt)? evt: ((event)? event:null); if (!isShiftKey(evt))
 return;  var key = getKeyCode(evt); if (key>40 || key<37)
 {
 doKeyDown_win1(evt); return; }
 ...
 {
 var gn = gridList_win1[i][0]; var nStart = gridList_win1[i][1]; var nStop = gridList_win1[i][2]; var gfields = gridFieldList_win1[i]; for (var j = 0; j < gfields.length; j++)
 {
 ...
 if (obj2)
 {
 ptCommonObj2.tryFocus(obj2); ptGridObj_win1.doScrollLeft(gn,ptCommonObj2.getLeftPos(obj2));  }
 return; }
...
{
if (evt && evt.button == 2) return; var oth=ptCommonObj2.getEO(evt);ptGridResizeObj_win1.gname = oth.id.split("#")[0];ptGridResizeObj_win1.fname = oth.id.substring(0,oth.id.length-1);var oEL =document.getElementById('th'+ ptGridResizeObj_win1.fname);if (!oEL || oEL == "undefined")
 return;var oTDsel=document.getElementById('pt_dragResize');var ogh_div=oth.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode; var oghl_div = document.getElementById('divghl'+ ptGridResizeObj_win1.gname);var oghr_div = document.getElementById('divghr'+ ptGridResizeObj_win1.gname);var ogbl_div = document.getElementById('divgbl'+ ptGridResizeObj_win1.gname);var ogbr_div = document.getElementById('divgbr'+ ptGridResizeObj_win1.gname);if (ptGridResizeObj_win1.lMin==0)
 ptGridResizeObj_win1.lMin=ptCommonObj2.getWidth(oghl_div);if (ogh_div && typeof ogh_div != 'undefined' && ogh_div.id && (ogh_div.id.substring(0,6) == 'divghr')) 
 ptGridResizeObj_win1.offset = ptCommonObj2.getWidth(oghl_div);ptGridResizeObj_win1.titleHeight=0;var tst=oTDsel.style;var ogc_div = document.getElementById('divgc'+ ptGridResizeObj_win1.gname); if (ptGridResizeObj_win1.fname.split("#")[1] == 1)
 tst.pixelLeft=oEL.offsetLeft+10;else if (!browserInfoObj2.isIE)
 tst.pixelLeft=oEL.offsetLeft + ptGridResizeObj_win1.offset+8; else
 tst.pixelLeft=oEL.offsetLeft + ptGridResizeObj_win1.offset + 15;tst.posWidth=oEL.offsetWidth; tst.pixelHeight=3;tst.display='block';ptGridResizeObj_win1.clientX = evt.clientX;ptGridResizeObj_win1.dragTD=true; oTDsel.style.height=40+Math.max(ptCommonObj2.getHeight(oghr_div),ptCommonObj2.getHeight(oghr_div))+Math.max(ptCommonObj2.getHeight(ogbr_div),ptCommonObj2.getHeight(ogbr_div));ptGridResizeObj_win1.TDcellWidth =oEL.offsetWidth;if (!browserInfoObj2.isIE)
{
oTDsel.style.width = ptGridResizeObj_win1.TDcellWidth;oTDsel.style.left = tst.pixelLeft;oTDsel.style.top=tst.pixelTop;}
...
{
 if(document.all)evt = event; if (typeof evt=='undefined') return; ptGridResizeObj_win1.TDcellWidth = ptGridResizeObj_win1.TDcellWidth + (evt.clientX - ptGridResizeObj_win1.clientX); ptGridResizeObj_win1.clientX = evt.clientX; ptGridResizeObj_win1.TDcellWidth=Math.max(ptGridResizeObj_win1.TDcellWidth, 28);  var oTDsel = document.getElementById('pt_dragResize'); oTDsel.style.posWidth = ptGridResizeObj_win1.TDcellWidth;  var gc_div_id = 'divgc'+ ptGridResizeObj_win1.gname; var ogc_div = document.getElementById(gc_div_id); var oEL =document.getElementById('th'+ ptGridResizeObj_win1.fname);   oTDsel.style.top = ptCommonObj2.getTopPos(oEL) - 8 + document.body.scrollTop; if (!browserInfoObj2.isIE)
 {
 oTDsel.style.width = ptGridResizeObj_win1.TDcellWidth; oTDsel.style.cursor='E-resize'; }
},
TDselUp : function (evt)
{ 

 var oTDsel = document.getElementById('pt_dragResize'); oTDsel.releaseCapture(); ptGridResizeObj_win1.TDcellWidth = Math.max(ptGridResizeObj_win1.TDcellWidth, 28); var oColH = document.getElementById('th'+ptGridResizeObj_win1.fname); var oColD = document.getElementById('td'+ptGridResizeObj_win1.fname); var oWidth=oColD.width; oColD.style.overflow = 'hidden'; oColD.width = ptGridResizeObj_win1.TDcellWidth - 4; var cWidth = oColD.width; oColH.style.overflow = 'hidden'; oColH.width = cWidth; var rOffset = cWidth - oWidth; if (ptGridResizeObj_win1.offset==0)
 { 
 var oth=document.getElementById('th'+ptGridResizeObj_win1.fname); var ogbl_div=document.getElementById('divgbl'+ptGridResizeObj_win1.gname); var ogbl_td = null; var oghl = null; var oghl_td =null; if (ogbl_div && typeof ogbl_div != 'undefined')
 {
 ogbl_td = ogbl_div.parentNode; ogbl_div.style.width=Math.max(ptCommonObj2.getWidth(ogbl_div)+rOffset,ptGridResizeObj_win1.lMin); ogbl_td.width =ogbl_div.style.width; oghl_div=document.getElementById('divghl'+ptGridResizeObj_win1.gname); oghl_td = oghl_div.parentNode;  oghl_div.style.width=ogbl_div.style.width; oghl_td.width =ogbl_div.style.width; }
 var ogbr_div=document.getElementById('divgbr'+ptGridResizeObj_win1.gname); var ogbr_td = null; var oghr_div = null; var oghr_td = null; if (ogbr_div && typeof ogbr_div != 'undefined')
 {
 ogbr_td = ogbr_div.parentNode;  ogbr_div.style.width=ptCommonObj2.getWidth(ogbr_div)-rOffset; ogbr_td.width =ogbr_div.style.width; oghr_div=document.getElementById('divghr'+ptGridResizeObj_win1.gname); oghr_td = oghr_div.parentNode;  oghr_div.style.width=ogbr_div.style.width; oghr_td.width =ogbr_div.style.width; } 
 } 
 
 oTDsel.style.display='none'; ptGridObj_win1.resizeItem(ptGridResizeObj_win1.fname,cWidth);},
colsrc : function(evt)
{
...
var ptGridObj_win1 = new PT_grid();function PT_gridDD()
{
...
PT_gridDD.prototype = {
 ...
 {
 if (!e && window.event)
 e = window.event; if (e.button == 2) return;     ptGridDDObj_win1.bMousedown = true;  ptGridDDObj_win1.idMousedown = this.id;  ptGridDDObj_win1.idDragged = this.getAttribute('dragableElement'); ptGridDDObj_win1.idDragged = ptGridDDObj_win1.idDragged + ''; if(ptGridDDObj_win1.idDragged=='')ptGridDDObj_win1.idDragged = this.dragableElement;   ptGridDDObj_win1.mouse_x = e.clientX; ptGridDDObj_win1.mouse_y = e.clientY;  ptGridDDObj_win1.cZIndex = ptGridDDObj_win1.cZIndex + 1;  ptGridDDObj_win1.CloneArr[ptGridDDObj_win1.idDragged].style.zIndex = ptGridDDObj_win1.cZIndex;  ptGridDDObj_win1.currentEl_allowX = ptGridDDObj_win1.srcArr[ptGridDDObj_win1.idDragged][2]; ptGridDDObj_win1.currentEl_allowY = ptGridDDObj_win1.srcArr[ptGridDDObj_win1.idDragged][3]; var parentEl = ptGridDDObj_win1.srcArr[ptGridDDObj_win1.idDragged][4]; ptGridDDObj_win1.drag_minX = false; ptGridDDObj_win1.drag_minY = false; ptGridDDObj_win1.drag_maxX = false; ptGridDDObj_win1.drag_maxY = false; if(parentEl){
 var obj = document.getElementById(parentEl); if(obj){
 ptGridDDObj_win1.drag_minX = ptCommonObj2.getLeftPos(obj); ptGridDDObj_win1.drag_minY = ptCommonObj2.getTopPos(obj); ptGridDDObj_win1.drag_maxX = ptGridDDObj_win1.drag_minX + obj.clientWidth; ptGridDDObj_win1.drag_maxY = ptGridDDObj_win1.drag_minY + obj.clientHeight;  } 
 }
 
 if(ptGridDDObj_win1.srcArr[ptGridDDObj_win1.idDragged][1]){
 ptGridDDObj_win1.CloneArr[ptGridDDObj_win1.idDragged].style.top = ptCommonObj2.getTopPos(ptGridDDObj_win1.srcArr[ptGridDDObj_win1.idDragged][0]) + 'px'; ptGridDDObj_win1.CloneArr[ptGridDDObj_win1.idDragged].style.left = ptCommonObj2.getLeftPos(ptGridDDObj_win1.srcArr[ptGridDDObj_win1.idDragged][0]) + 'px'; }
 ptGridDDObj_win1.el_x = ptGridDDObj_win1.CloneArr[ptGridDDObj_win1.idDragged].style.left.replace('px','')/1; ptGridDDObj_win1.el_y = ptGridDDObj_win1.CloneArr[ptGridDDObj_win1.idDragged].style.top.replace('px','')/1;  return false; },
 __timerDragDropElement : function()
 {
 window.thisRef = this;        this.srcArr[this.idDragged][0].style.visibility = 'hidden';  if(this.srcArr[ptGridDDObj_win1.idDragged][5]){
 var id1 = this.CloneArr[this.idDragged].id + ''; var id2 = this.srcArr[this.idDragged][0].id + ''; var string = this.srcArr[ptGridDDObj_win1.idDragged][5] + '("' + id1 + '","' + id2 + '")'; eval(string); } 
 
 }, 
 __cancelSelectionEvent : function()
 {
 if (ptGridDDObj_win1.bMousedown) return false;   return true; },
 __moveDragableElement : function(e) 
 {
 if (!e && window.event)
 e = window.event;  var xdiff=0, ydiff=0;  if (ptGridDDObj_win1.bMousedown) 
 {
 xdiff=e.clientX - ptGridDDObj_win1.mouse_x;  ydiff=e.clientY - ptGridDDObj_win1.mouse_y;  if (xdiff != 0 || ydiff != 0) {
 ptGridDDObj_win1.bMove=true;  ptGridDDObj_win1.__timerDragDropElement();   }
 else return; }
 else return; var dragObj = ptGridDDObj_win1.CloneArr[ptGridDDObj_win1.idDragged]; if(ptGridDDObj_win1.currentEl_allowX && xdiff != 0){ 
 var leftPos = (xdiff+ ptGridDDObj_win1.el_x); if(ptGridDDObj_win1.drag_maxX){
 var tmpMaxX = ptGridDDObj_win1.drag_maxX - dragObj.offsetWidth; if(leftPos > tmpMaxX)leftPos = tmpMaxX
 ...
 {
 var sScript=ptGridObj_win1.doSort(sId); return sScript; },

 __stop_dragDropElement : function(e) 
 {
 if (!ptGridDDObj_win1.bMove) { 
 
 if (ptGridDDObj_win1.bMousedown) { 
 ptGridDDObj_win1.bMousedown = false;  var idMousedwn= ptGridDDObj_win1.idMousedown; ptGridDDObj_win1.idMousedown = ""; var tmp_js = ptGridDDObj_win1.__getSortJScript(idMousedwn); if (tmp_js != "") 
 eval(tmp_js); }
 return;  }
 
 
 ptGridDDObj_win1.bMousedown = false;  ptGridDDObj_win1.bMove = false;  ptGridDDObj_win1.idMousedown = "";  if (!e && window.event)
 e = window.event;   dropDestination = ptCommonObj2.getEO(e); if (dropDestination.nodeType == 3) 
 dropDestination = dropDestination.parentNode; var trgNo=ptGridDDObj_win1.getValidTargetNo(ptGridDDObj_win1.srcArr[ptGridDDObj_win1.idDragged][0].id,e); if (trgNo!=-1) 
 eval(ptGridDDObj_win1.trgArr[trgNo][1] + '("' + ptGridDDObj_win1.srcArr[ptGridDDObj_win1.idDragged][0].id + '","' + ptGridDDObj_win1.trgArr[trgNo][0].id + '",' + e.clientX + ',' + e.clientY + ')'); else 
 ptGridDDObj_win1.__slideElementBackIntoItsOriginalPosition(ptGridDDObj_win1.idDragged);    ptGridDDObj_win1.idDragged = false; oDragTxt = document.getElementById("pt_dragtxt"); oDragTxt.innerHTML = ""; oDragTxt.style.display="none";   },
 __slideElementBackIntoItsOriginalPosition : function(numId)
 {
 
 var currentX = this.CloneArr[numId].style.left.replace('px','')/1; var currentY = this.CloneArr[numId].style.top.replace('px','')/1;   var targetX = ptCommonObj2.getLeftPos(ptGridDDObj_win1.srcArr[numId][0]); var targetY = ptCommonObj2.getTopPos(ptGridDDObj_win1.srcArr[numId][0]);;  if(this.slideBack){
...
var ptGridDDObj_win1 = null;var preID_win1=null; var orgColor_win1="white"; var frozn_win1="f"; var rowSelColor_win1=""; function resetVars_win1()
{
 if(typeof(preID_win1)!='undefined' && preID_win1 != null)
 HighLightTR(rowSelColor_win1, orgColor_win1, preID_win1, 1); }
...
function HighlightDone(sId)
{
 if(typeof(preID_win1)!='undefined' && preID_win1 != null) 
 {
 var saveID = preID_win1; var newID = sId; if (preID_win1.substr(0,1).indexOf(frozn_win1)==0) 
 saveID = preID_win1.substr(1); if (sId.substr(0,1).indexOf(frozn_win1)==0) 
...
function HighLightTR(sRowSelColor, sRowOrgColor, sId, bRun) 
{
 ...
 if (rowSelColor_win1=="") 
 rowSelColor_win1 = sRowSelColor;  for (var i=0;i<2;i++) 
 {
 var obj; if (i== 0)
 obj=document.getElementById(preID_win1); else if (preID_win1.substr(0,1).indexOf(frozn_win1)==0) 
 obj=document.getElementById(preID_win1.substr(1));  else
 obj=document.getElementById(frozn_win1.concat(preID_win1));  if(typeof(obj) =='undefined' || obj==null)
 break; var x = obj.childNodes; for (var j=0;j<x.length;j++)
 {
 if((x[j].nodeName).indexOf("TD")==0 && (x[j].parentNode.nodeName).indexOf("TR") == 0)
 x[j].style.backgroundColor=orgColor_win1;  }
 }
 
 for (var i=0;i<2;i++) 
 {
 var obj; if (i== 0)
 obj=document.getElementById(sId); else if (sId.substr(0,1).indexOf(frozn_win1)==0)
 obj=document.getElementById(sId.substr(1)); else
 obj=document.getElementById(frozn_win1.concat(sId)); if(typeof(obj) =='undefined' || obj==null)
 ...
 preID_win1 = sId;  orgColor_win1 = sRowOrgColor;}
